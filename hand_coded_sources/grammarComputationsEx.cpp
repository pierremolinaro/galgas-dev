...
#include "FIRST_computationsEx.h"
...
cProductionEx::cProductionEx (void) :
mSourceFileName (),
aLigneDefinition (0),
...
cPureBNFproductionsListEx::cPureBNFproductionsListEx (void) :
...
void swap (cProductionEx & ioProduction1, cProductionEx & ioProduction2) {
...
static bool
searchForIdenticalProductions (const cPureBNFproductionsListEx & productions,
  ...
  for (PMSInt32 i=0 ; i<productions.length () ; i++) {
    const cProductionEx & pi = productions (i COMMA_HERE) ;
    for (PMSInt32 j=i+1 ; j<productions.length () ; j++) {
      const cProductionEx & pj = productions (j COMMA_HERE) ;
...
static PMUInt16 bddBitCountForVocabulary (const cVocabularyEx & inVocabulary) {
...
//--- Building pure BNF productions ---------------------------------------------------------------------
  ...
  #endif
  cVocabularyEx vocabulary ;
  cPureBNFproductionsListEx pureBNFproductions ;
...
void
routine_grammarAnalysisAndGeneration (C_CompilerEx & inLexique,
                                      const GGS_lstring inTargetFileName,
                                      const GGS_lstring inGrammarClass,
                                      const GGS_uint inOriginalGrammarStartSymbol,
