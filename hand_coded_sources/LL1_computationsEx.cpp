...
//--- Attributs
  ...
  protected : const cVocabularyEx & mVocabulary ;
...
static bool
check_LL1_condition (const cPureBNFproductionsListEx & inPureBNFproductions,
    ...
    const PMSInt32 first = inPureBNFproductions.tableauIndicePremiereProduction (i COMMA_HERE) ;
      ...
      //--- Calculer et afficher les premiers
        ...
        for (PMSInt32 j=first ; j<=derniere ; j++) {
          ...
          cProductionEx & p = inPureBNFproductions (numeroProduction COMMA_HERE) ;
...
static void
engendrerAiguillageNonTerminaux (const cVocabularyEx & inVocabulary,
...
static void
printProductions (const cPureBNFproductionsListEx & inPureBNFproductions,
                  const cVocabularyEx & inVocabulary,
  ...
  if (firstProduction >= 0) {
    ...
    for (PMSInt32 j=firstProduction ; j<=lastProduction ; j++) {
      ...
      cProductionEx & p = inPureBNFproductions (inPureBNFproductions.tableauIndirectionProduction (j COMMA_HERE) COMMA_HERE) ;
      C_String title ;
...
static void
printDecisionTable (const cPureBNFproductionsListEx & inPureBNFproductions,
                    const cVocabularyEx & inVocabulary,
                    const C_String & inLexiqueName,
                    const PMSInt32 inNonterminalIndex,
