...
//--- Get transitions LR1 item set from a state for a symbol
  public : void
  getTransitionFrom (const cPureBNFproductionsListEx & inProductionRules,
                     const PMSInt32 inSymbol,
                     c_LR1_items_set & out_LR1_item_set) ;

//--- Closing the LR1 items set
  public : void
  close_LR1_items_set (const cPureBNFproductionsListEx & inProductionRules,
...
//--- Display LR1 items set
  public : void
  display (const cPureBNFproductionsListEx & inProductionRules,
...
//--- Search from a LR1 items set (used for building 'reduce' actions of LR table)
  public : void
  getProductionsWhereLocationIsRight (const cPureBNFproductionsListEx & inProductionRules,
...
close_LR1_items_set (const cPureBNFproductionsListEx & inProductionRules,
  ...
  for (PMSInt32 i=0 ; i<mItemsSet.count () ; i++) {
    ...
    const cProductionEx & p = inProductionRules (productionRule COMMA_HERE) ;
...
display (const cPureBNFproductionsListEx & inProductionRules,
         ...
         C_HTML_FileWrite & inHTMLfile) const {
  for (PMSInt32 i=0 ; i<mItemsSet.count () ; i++) {
    const cProductionEx & p = inProductionRules (mItemsSet (i COMMA_HERE).mProductionRuleIndex COMMA_HERE) ;
...
getTransitionFrom (const cPureBNFproductionsListEx & inProductionRules,
  ...
  for (PMSInt32 i=0 ; i<mItemsSet.count () ; i++) {
    ...
    const cProductionEx & p = inProductionRules (productionRuleIndex COMMA_HERE) ;
    const PMSInt32 location = mItemsSet (i COMMA_HERE).mLocationIndex ;
    if (location < p.aDerivation.count ()) {
...
//--- Get transitions LR1 item set from a state for a symbol
  public : void getTransitionFrom (const cPureBNFproductionsListEx & inProductionRules,
                                   const PMSInt32 inStateIndex,
                                   const PMSInt32 inSymbol,
                                   c_LR1_items_set & out_LR1_item_set) ;

//--- Display LR1 items set
  public : void display (const cPureBNFproductionsListEx & inProductionRules,
                         const cVocabularyEx & inVocabulary,
                         C_HTML_FileWrite & inHTMLfile) const ;
...
void c_LR1_items_sets_collection::getTransitionFrom (const cPureBNFproductionsListEx & inProductionRules,
                                                     const PMSInt32 inStateIndex,
                                                     const PMSInt32 inSymbol,
                                                     c_LR1_items_set & out_LR1_item_set) {
...
generate_LR1_grammar_cpp_file (C_CompilerEx & inLexique,
                               const cPureBNFproductionsListEx & inProductionRules,
                               const cVocabularyEx & inVocabulary,
                               const TC_UniqueArray2 <cDecisionTableElementEx> & inSLRdecisionTable,
...
//--- State action tables
  ...
  for (PMSInt32 i=0 ; i<rowsCount ; i++) {
    ...
    for (PMSInt32 j=0 ; j<columnsCount ; j++) {
      ...
      const cDecisionTableElementEx::enumDecision decision = inSLRdecisionTable (i, j COMMA_HERE).decision () ;
      if (decision != cDecisionTableElementEx::kUndefinedState) {
        startIndex += 2 ;
        generatedZone3 << "\n" ;
        if (isFirst) {
          isFirst = false ;
        ...
        generatedZone3 << "C_Lexique_" << inLexiqueName.identifierRepresentation () << "::kToken_"
                       ...
                       << ", " ;
        if (decision == cDecisionTableElementEx::kDecisionReduce) { // Reduce action
          generatedZone3 << "REDUCE (" << cStringWithSigned (parameter) << ")" ;
        }else if (decision == cDecisionTableElementEx::kDecisionShift) { // Shift action
...
static void
compute_LR1_automation (const cPureBNFproductionsListEx & inProductionRules,
                        const cVocabularyEx & inVocabulary,
                        const TC_UniqueArray <TC_UniqueArray <PMSInt32> > & inFIRSTarray,
...
//--- Build LR1 table... detect if grammar is not LR1
  const PMSInt32 terminalSymbolsCount = inVocabulary.getTerminalSymbolsCount () ;
  TC_UniqueArray2 <cDecisionTableElementEx> SLRdecisionTable (LR1_items_sets_collection->getStateCount (), terminalSymbolsCount) ;
...
//--- Shift actions
  for (PMSInt32 index=0 ; index<transitionList.length () ; index++) {
    if (transitionList (index COMMA_HERE).mAction < terminalSymbolsCount) {
      ...
      SLRdecisionTable (sourceState, terminal COMMA_HERE) = cDecisionTableElementEx::shiftDecision (targetState) ;
      shiftActions ++ ;
      if (inHTMLfile != NULL) {
        inHTMLfile->outputRawData ("<tr class=\"result_line\"><td class=\"result_line\"><code>") ;
...
//--- Reduce actions
  ...
  //--- Accept condition
    ...
    if (acceptCondition) {
      ...
      SLRdecisionTable (state, terminal COMMA_HERE) = cDecisionTableElementEx::acceptDecision () ;
    }
  //--- Reduce
    for (PMSInt32 p=0 ; p<productionsSet.count () ; p++) {
      ...
      if (leftNonTerminal != (inVocabulary.getAllSymbolsCount () - 1)) {
        ...
        SLRdecisionTable (state, terminal COMMA_HERE) = cDecisionTableElementEx::reduceDecision (productionIndex) ;
      }
    }
  }
  if (inHTMLfile != NULL) {
