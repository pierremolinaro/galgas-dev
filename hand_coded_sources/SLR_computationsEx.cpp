...
//--- Closing the LR0 items set
  public : void close_items_set (const cPureBNFproductionsListEx & inProductionRules,
...
//--- Search from a LR0 items set (used for building 'reduce' actions of SLR table)
  public : void getProductionsWhereLocationIsRight (const cPureBNFproductionsListEx & inProductionRules,
...
close_items_set (const cPureBNFproductionsListEx & inProductionRules,
                 const PMSInt32 inTerminalSymbolsCount) {
  bool loop = true ;
  while (loop) {
    loop = false ;
  ...
  while (nonTerminal.hasCurrentObject ()) {
    ...
    while (currentAltForNonTerminal2.hasCurrentObject ()) {
      generatedZone3 << "void cGrammar_" << inTargetFileName.identifierRepresentation ()
                     ...
                     << " (" ;
      const PMSInt32 pureBNFleftNonterminalIndex = (PMSInt32) nonTerminal._mNonTerminalIndex (HERE).mValue ;
...
static void
compute_LR0_automation (const cPureBNFproductionsListEx & inProductionRules,
                        const cVocabularyEx & inVocabulary,
...
//--- Build SLR table... detect if grammar is not SLR
  const PMSInt32 terminalSymbolsCount = inVocabulary.getTerminalSymbolsCount () ;
  TC_UniqueArray2 <cDecisionTableElementEx> SLRdecisionTable (LR0_items_sets_collection.getStatesCount (), terminalSymbolsCount) ;
