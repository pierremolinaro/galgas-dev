...
\item \ggs=%replaceBy=, qui suggère de remplacer le token désigné par l'\ggs=expression= indiquée~;
\item \ggs=%insertAfter=, qui suggère d'insérer l'\ggs=expression= indiquée après le token désigné~;
\item \ggs=%insertBefore=, qui suggère d'insérer l'\ggs=expression= indiquée avant le token désigné.
\end{itemize}
...
\begin{galgas}
fixit { %replaceBy "toto" } # Suggère de remplacer le token par toto
fixit { %replaceBy "toto" %replaceBy "tata" } # Deux suggestions
fixit { %replaceBy @stringlist {!"toto" !"tata"} } # Identique au précédent
fixit { %replaceBy {!"toto" !"tata"} } # Le type @stringlist est inféré
\end{galgas}
...
La clause \ggs=fixit= contient une séquence de suggestions, qui peut être vide. Une séquence peut contenir plusieurs suggestions \ggs=%replaceBy=, \ggs=%insertAfter=, \ggs=%insertBefore=~; par contre, la suggestion \ggs=%remove= ne peut apparaître qu'au plus une fois. Les suggestions sont présentées à l'utilisateur dans l'ordre où elles apparaissent dans la clause \ggs=fixit=. Il est parfaitement légal d'écrire par exemple~:
\begin{galgas}
fixit {
  %replaceBy "toto"
  %insertAfter "tata"
...
\begin{itemize}
  ...
  \item un exemple \ggs=%replaceBy= (\refSubsubsectionPage {troisiemeExempleFixIt})~;
  \item un second exemple \ggs=%replaceBy=, qui met en évidence un piège qui existe avec des chaînes de caractères (\refSubsubsectionPage {quatriemeExempleFixIt}).
\end{itemize}

L'utilisation de \ggs=before= et de \ggs=after= est analogue à celui de \ggs=%replaceBy=~: on se reportera aux exemples \ggs=%replaceBy= pour ceux-ci.
...
\subsubsectionLabel{Exemple \texttt{fixit \%replaceBy}}{troisiemeExempleFixIt}
...
On pourrait améliorer le message d'erreur ci-dessus en ajoutant la liste des valeurs possibles. Ajouter une clause \ggs=fixit %replaceBy= permet de le faire très simplement. Après le mot réservé \ggs=%replaceBy= est attendue une expression de type \ggs=@string= ou \ggs=@stringlist=, or la variable \ggs=valeursPossibles= est du type \ggs=@stringset=~: on utilise donc le getter \refGetterPage{stringset}{stringList}. D'où~:
...
\begin{galgas}
  ...
  if [valeursPossibles hasKey !valeurÀVérifier] then
    ...
    error valeurÀVérifier : "valeur invalide" fixit {
      %replaceBy [valeursPossibles stringList]
    }
  end
}
\end{galgas}

\subsubsectionLabel{Exemple \texttt{fixit \%replaceBy}, chaîne de caractères}{quatriemeExempleFixIt}
...
\begin{galgas}
case . "monExtension"
%errorMessage "un fichier source"
%useGrammar maGrammaire
...
\begin{galgas}
with expression_cle prefixe_optionnel in expression_table
%errorMessage methode_recherche
do
  liste_instructions_do
end
\end{galgas}
...
map @maTable {
  @uint mIndex
  %insertSetter insertKey %errorMessage "entree deja presente"
}
\end{galgas}
...
map @maTable {
  ...
  search searchKey %errorMessage "entree %K absente"
}
\end{galgas}
...
@lstring cle = ...
@uint occurenceCount
with cle in table %errorMessage searchKey do
...
@lstring cle = ...
@uint occurenceCount
with cle : abc_ in table %errorMessage searchKey do
