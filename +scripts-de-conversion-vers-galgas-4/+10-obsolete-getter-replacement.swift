...
class SupprimerSelecteurInutile : Correcteur {
  var préfixeMessageErreur : String { "semantic error #1: obsolete getter" }
  ...
  func corriger (_ lignesDuFichier : inout [String],
    ...
    print ("  First fixit line '\(firstFixItLine)'")
    let firstFixitLineComponents = firstFixItLine.components (separatedBy: " ")
    var replacementGetter = firstFixitLineComponents.last!
    replacementGetter.removeFirst ()
    print ("  Retplacement getter '\(replacementGetter)'")
    let ligneModifiée = préfixe + replacementGetter + suffixe
  ...
  //-------------------- Compiler
    ...
    while loop {
      ...
      let (s, status) = runHiddenCommand (
        ...
        ["--no-color", "--max-errors=1", "--max-warnings=1", "--error-on-obsolete-getter-call", inCheminFichierGALGAS]
      )
      if status == 0 {
