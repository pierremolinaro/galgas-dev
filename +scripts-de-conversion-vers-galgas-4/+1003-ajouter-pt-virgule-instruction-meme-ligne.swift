...
class AjouterPointVirguleEntreInstructions : Correcteur {
  var préfixeMessageErreur : String { "semantic error #1: instructions on same line should be separated by ';'" }
  ...
  func corriger (_ lignesDuFichier : inout [String],
    ...
    préfixe.removeLast (ligneConcernée.count - premierCaractère - 1)
...
//----------------------------------------------------------------------------------------
  ...
  func traduire (projetGALGAS inCheminFichierGALGAS : String) -> (Bool, Int) {
    let correcteurs : [Correcteur] = [AjouterPointVirguleEntreInstructions ()]
...
while let file = directoryEnumerator?.nextObject () as? String {
  if file.hasSuffix (".ggsproject") {
    ...
    print ("  found \(path)")
  }else if file.hasSuffix (".galgasproject") || file.hasSuffix (".galgasProject") {
    let path = scriptDir.appending("/\(file)")
    galgasProjectFiles.append (path)
    print ("  found \(path)")
  }
