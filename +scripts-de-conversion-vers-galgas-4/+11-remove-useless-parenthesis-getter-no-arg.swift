...
class SupprimerSelecteurInutile : Correcteur {
  var préfixeMessageErreur : String { "semantic error #1: getter with no argument, remove parenthesis" }
...
//----------------------------------------------------------------------------------------
  ...
  func traduire (projetGALGAS inCheminFichierGALGAS : String) -> (Bool, Int) {
    let correcteurs : [Correcteur] = [SupprimerSelecteurInutile ()]
  ...
  //-------------------- Compiler
    ...
    while loop {
      ...
      let (s, status) = runHiddenCommand (
        ...
        ["--no-color", "--max-errors=1", "--max-warnings=1", "--error-on-getter-call-with-no-argument", inCheminFichierGALGAS]
      )
      if status == 0 {
