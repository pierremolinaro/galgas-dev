...
pr_galgas_component_scanner_parser_233_6_ (galgas_scanner & _inLexique,
  ...
  _inLexique.acceptTerminal (ACCEPT_TERMINAL (galgas_scanner::galgas_scanner_1_lexique) COMMA_HERE) ;
  _inLexique.acceptTerminal (ACCEPT_TERMINAL (galgas_scanner::galgas_scanner_1__3B) COMMA_HERE) ;
  const GGS_typeTableMessagesErreurs _var_10795 = var_cas_table_messages ;
  const GGS_typeTableMessagesErreurs::element_type * operand_10795 = _var_10795.firstObject () ;
  while ((operand_10795 != NULL)) {
    macroValidPointer (operand_10795) ;
    if (((var_cas_usedErrorMessageSet.reader_hasKey (_inLexique, operand_10795->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (307)) COMMA_SOURCE_FILE_AT_LINE (307)))._operator_not ()).isBuiltAndTrue ()) {
      operand_10795->mKey.reader_location (_inLexique COMMA_HERE).signalGGSSemanticWarning (_inLexique, GGS_string (true, "this error message is not used") COMMA_SOURCE_FILE_AT_LINE (308)) ;
    }
    operand_10795 = operand_10795->nextObject () ;
  }
