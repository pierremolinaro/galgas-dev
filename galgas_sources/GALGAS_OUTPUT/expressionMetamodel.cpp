...
GGS_filewrapperObjectInstanciationInExpression (cPtr__AC_galgas_class * inPointer) {
  macroAssignPointer (mPointer, inPointer) ;
}
...
_castFrom (C_Compiler & inLexique,
           ...
           const GGS_location & inErrorLocation
           COMMA_LOCATION_ARGS) {
  GGS_filewrapperObjectInstanciationInExpression _result ;
  if (inPointer != NULL) {
    macroValidPointer (inPointer) ;
    const bool ok = inUseKindOfClass
    ...
    if (ok) {
      _result = GGS_filewrapperObjectInstanciationInExpression (inPointer) ;
    }else{
      inErrorLocation.signalCastError (inLexique,
