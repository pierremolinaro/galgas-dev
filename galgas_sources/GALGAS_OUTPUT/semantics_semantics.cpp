...
isEqualToMapElement (const AC_galgas_map_element * inOperand) const {
  ...
  macroValidPointer (_p) ;
  return (mInfo.mFormalArgumentList == _p->mInfo.mFormalArgumentList).boolValue () ;
}
...
_insertElement (C_Lexique & inLexique,
  ...
  if (_isBuilt ()
   ...
   && inKey._isBuilt ()) {
    insulateMap () ;
    e_treewalkingRoutineHeaderMap info  ;
    info.mFormalArgumentList = inParameter0 ;
    GGS_location existingKeyLocation ;
    internalInsert (inKey, (void *) & info, mSharedMapRoot->rootForKey (inKey), elementID, existingKeyLocation) ;
    if (elementID < 0) {
      emitInsertMapSemanticErrorMessage (inLexique, inKey, inErrorMessage, existingKeyLocation COMMA_THERE) ;
...
_searchElement (C_Lexique & inLexique,
  ...
  if (node == NULL) {
    ...
    outParameter0 = node->mInfo.mFormalArgumentList ;
    if (outIndex != NULL) {
      * outIndex = GGS_luint (GGS_uint (true, (uint32) node->mID), inKey) ;
    }
  }
...
method_searchKey (C_Lexique & inLexique,
  ...
  _searchElement (inLexique,
                  "the '%K' treewalking routine property is not declared",
                  inKey,
                  outParameter0,
                  NULL
                  COMMA_THERE) ;
