//---------------------------------------------------------------------------*
//                                                                           *
//                 File 'include_typeInstructionLexicale.h'                  *
//         Generated by version GALGAS_BETA_VERSION (LL(1) grammar)          *
//                      march 13th, 2006, at 19h14'53"                       *
//                                                                           *
//---------------------------------------------------------------------------*

//--- START OF USER ZONE 1


//--- END OF USER ZONE 1

//---------------------------------------------------------------------------*
//                                                                           *
//              abstract class 'cPtr_typeInstructionLexicale'                *
//                                                                           *
//---------------------------------------------------------------------------*

#ifndef typeInstructionLexicale_DEFINED
#define typeInstructionLexicale_DEFINED

//---------------------------------------------------------------------------*

class cPtr_typeInstructionLexicale : public GGS_class {
//--- START OF USER ZONE 2

public : virtual bool instruction__uses_loop_variable (void) const = 0 ;
public : virtual void generate_scanner_instruction (const C_String & inLexiqueName,
                                                    AC_OutputStream & inCppFile) const = 0 ;

//--- END OF USER ZONE 2

  public : cPtr_typeInstructionLexicale (LOCATION_ARGS) ;
} ;

//---------------------------------------------------------------------------*

class GGG_typeInstructionLexicale : public C_GGS_object {
  public : GGG_typeInstructionLexicale (LOCATION_ARGS) ;
} ;

//---------------------------------------------------------------------------*

#endif

