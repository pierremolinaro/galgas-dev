...
appendForMapDescription (C_Compiler & _inLexique,
  ...
  ioString << "|-value " ;
  ioString.appendSigned (inElementIndex) ;
  ioString << ":" << mInfo.mReplacementFunction.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
isEqualToMapElement (const AC_galgas_map_element * inOperand) const {
  ...
  return (mInfo.mReplacedString._operator_isEqual (_p->mInfo.mReplacedString)).boolValue ()
           && (mInfo.mReplacementFunction._operator_isEqual (_p->mInfo.mReplacementFunction)).boolValue () ;
}
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  ...
  if (equal) {
    const cPtr_typeUnicodeCharSet * _p = dynamic_cast <const cPtr_typeUnicodeCharSet *> (inOperand) ;
    macroValidPointer (_p) ;
    equal = mUnicodeCharSetName._operator_isEqual (_p->mUnicodeCharSetName).boolValue () ;
  }
  return equal ;
}
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString << "->@typeUnicodeCharSet:"
           << mUnicodeCharSetName.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  bool equal = typeid (this) == typeid (inOperand) ;
  if (equal) {
    const cPtr_typeInstructionLexicalWarning * _p = dynamic_cast <const cPtr_typeInstructionLexicalWarning *> (inOperand) ;
    macroValidPointer (_p) ;
    equal = mErrorMessageIndex._operator_isEqual (_p->mErrorMessageIndex).boolValue () ;
  }
  return equal ;
