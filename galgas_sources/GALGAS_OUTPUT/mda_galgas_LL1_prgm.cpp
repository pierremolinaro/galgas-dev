    ...
    try{
      ...
      for (sint32 i=0 ; i<sourceFilesArray.count () ; i++) {
        ...
        if (fileExtension.compare ("ggs") == 0) {
          ...
          const GGS_lstringlist _var_7915 = var_cas_fileList ;
          const GGS_lstringlist::element_type * operand_7915 = _var_7915.firstObject () ;
          while ((operand_7915 != NULL)) {
            macroValidPointer (operand_7915) ;
            _inLexique.printMessage (((GGS_string (true, "----- Compiling '"))._operator_concat (operand_7915->mValue))._operator_concat (GGS_string (true, "'\n")) COMMA_SOURCE_FILE_AT_LINE (188)) ;
            GGS_string var_cas_extension ;
            var_cas_extension = operand_7915->mValue.ggs_string ().reader_pathExtension (_inLexique COMMA_SOURCE_FILE_AT_LINE (189)) ;
            GGS_lstring  var_cas_fullFilePath ;
            var_cas_fullFilePath = GGS_lstring::constructor_new (_inLexique, ((var_cas_inSourceFile.ggs_string ().reader_stringByDeletingLastPathComponent (_inLexique COMMA_SOURCE_FILE_AT_LINE (191)))._operator_concat (GGS_string (true, "/")))._operator_concat (operand_7915->mValue.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (191))), operand_7915->mValue.reader_location (_inLexique COMMA_SOURCE_FILE_AT_LINE (192)) COMMA_HERE) ;
            if (((var_cas_extension)._operator_isEqual (GGS_string (true, "gTreewalking"))).isBuiltAndTrue ()) {
              ...
              operand_7915->mValue.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "unhandled extension") COMMA_SOURCE_FILE_AT_LINE (213)) ;
            }
            operand_7915 = operand_7915->nextObject () ;
          }
          if (verboseOptionOn || ((_inLexique.totalErrorCount () + _inLexique.totalWarningCount ()) > 0)) {
            co << "Analysis of '" << sourceFilesArray (i COMMA_HERE).lastPathComponent () << "' completed. " ;
            switch (_commonLexique->totalErrorCount ()) {
