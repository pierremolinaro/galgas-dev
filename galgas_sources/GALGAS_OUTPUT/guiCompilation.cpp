...
template_filewrapper_guiGenerationTemplates_gui_implementation (C_Compiler & inLexique,
  ...
  if (isBuilt) {
    ...
    GGS_extensionMap::cEnumerator enumerator_6592 (var_cas_GUI_CONTEXT.reader_mExtensionMap (inLexique COMMA_SOURCE_FILE_AT_LINE (148)), true) ;
    const GGS_extensionMap::cElement * operand_6592 = NULL ;
    if (enumerator_6592.hasCurrentObject ()) {
      result << "  if" ;
      while (((operand_6592 = enumerator_6592.nextObject ()))) {
        macroValidPointer (operand_6592) ;
        result << " ([inExtension isEqualToString:@" ;
        result << operand_6592->mKey.ggs_string ().reader_utf8Representation (inLexique COMMA_SOURCE_FILE_AT_LINE (150)) ;
        result << "]) {\n"
          "    result = [[[OC_Tokenizer_" ;
        result << operand_6592->mInfo.mLexiqueName ;
        result << " alloc] init] autorelease] ;\n" ;
        if (enumerator_6592.hasNextObject ()) {
          result << "  }else if" ;
        }
      }
      result << "  }\n" ;
    ...
    result << "  return result ;\n"
      ...
      "  return [NSArray arrayWithObjects:\n" ;
    GGS_importedLexiqueList::cEnumerator enumerator_7016 (var_cas_GUI_CONTEXT.reader_mWithLexiqueList (inLexique COMMA_SOURCE_FILE_AT_LINE (162)), true) ;
    const GGS_importedLexiqueList::cElement * operand_7016 = NULL ;
    while (((operand_7016 = enumerator_7016.nextObject ()))) {
      macroValidPointer (operand_7016) ;
      result << "    [[[OC_Tokenizer_" ;
      result << operand_7016->mLexiqueClassName ;
      result << " alloc] init] autorelease],\n" ;
    }
    result << "    nil\n"
      "  ] ;\n"
