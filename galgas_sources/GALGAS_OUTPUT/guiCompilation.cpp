...
GALGAS_string filewrapperTemplate_guiGenerationTemplates_gui_5F_implementation (C_Compiler * inCompiler,
  ...
  if (in_GUI_5F_CONTEXT.reader_mWithLexiqueList (SOURCE_FILE_AT_LINE (58)).isValid ()) {
    ...
    while (enumerator_2254.hasCurrentObject ()) {
      ...
      result << "] = {\n" ;
      GALGAS_uint index_5410_ (0) ;
      if (enumerator_2254.current_mTextMacroList (HERE).isValid ()) {
        cEnumerator_textMacroList enumerator_5410 (enumerator_2254.current_mTextMacroList (HERE), kEnumeration_up) ;
        while (enumerator_5410.hasCurrentObject ()) {
          result << "    @" ;
          result << enumerator_5410.current_mKey (HERE).reader_utf_38_Representation (SOURCE_FILE_AT_LINE (116)).stringValue () ;
          result << ",\n" ;
          index_5410_.increment () ;
      ...
      if (enumerator_2254.current_mTextMacroList (HERE).isValid ()) {
        cEnumerator_textMacroList enumerator_5823 (enumerator_2254.current_mTextMacroList (HERE), kEnumeration_up) ;
        while (enumerator_5823.hasCurrentObject ()) {
          result << "    @" ;
          result << enumerator_5823.current_mContents (HERE).reader_utf_38_Representation (SOURCE_FILE_AT_LINE (126)).stringValue () ;
          result << ",\n" ;
  ...
  if (in_GUI_5F_CONTEXT.reader_mExtensionMap (SOURCE_FILE_AT_LINE (141)).isValid ()) {
    cEnumerator_extensionMap enumerator_6343 (in_GUI_5F_CONTEXT.reader_mExtensionMap (SOURCE_FILE_AT_LINE (141)), kEnumeration_up) ;
    const bool nonEmpty_enumerator_6343 = enumerator_6343.hasCurrentObject () ;
    if (nonEmpty_enumerator_6343) {
      result << "  if" ;
    }
    while (enumerator_6343.hasCurrentObject ()) {
      result << " ([inExtension isEqualToString:@" ;
      result << enumerator_6343.current_lkey (HERE).mAttribute_string.reader_utf_38_Representation (SOURCE_FILE_AT_LINE (143)).stringValue () ;
      result << "]) {\n"
        "    result = [[[OC_Tokenizer_" ;
      result << enumerator_6343.current_mLexiqueName (HERE).stringValue () ;
      result << " alloc] init] autorelease] ;\n" ;
      if (enumerator_6343.hasNextObject ()) {
        result << "  }else if" ;
      }
  ...
  if (in_GUI_5F_CONTEXT.reader_mWithLexiqueList (SOURCE_FILE_AT_LINE (155)).isValid ()) {
    cEnumerator_importedLexiqueList enumerator_6746 (in_GUI_5F_CONTEXT.reader_mWithLexiqueList (SOURCE_FILE_AT_LINE (155)), kEnumeration_up) ;
    while (enumerator_6746.hasCurrentObject ()) {
      result << "    [[[OC_Tokenizer_" ;
      result << enumerator_6746.current_mLexiqueClassName (HERE).stringValue () ;
      result << " alloc] init] autorelease],\n" ;
