...
_castFrom (C_Compiler & inLexique,
           ...
           COMMA_LOCATION_ARGS) {
  GGS_listmapDeclaration _result ;
  if (inPointer != NULL) {
    ...
    if (ok) {
      _result = GGS_listmapDeclaration (inPointer) ;
    }else{
...
void GGS_parsedSemanticsComponentMap::
modifier_setMRootForKey (C_Compiler & inLexique,
  ...
  if (isBuilt () && inValue.isBuilt () && inKey.isBuilt ()) {
    ...
    if (node == NULL) {
      ...
      errorMessage << "the '" << inKey << "' key does not exist when calling 'setMRootForKey' modifier" ;
      inLexique.onTheFlyRunTimeError (errorMessage COMMA_THERE) ;
    }else{
      node->mInfo.mRoot = inValue ;
    }
  }
}
