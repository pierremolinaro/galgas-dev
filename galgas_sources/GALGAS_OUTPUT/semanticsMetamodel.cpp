...
isEqualToObject (const cListElement * inOperand) const {
  ...
  if (! equal) {
    ...
    equal = mAttributeTypeName._operator_isEqual (_p->mAttributeTypeName).boolValue ()
         ...
         && mFeatureList._operator_isEqual (_p->mFeatureList).boolValue () ;
  }
  return equal ;
}
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString.writeStringMultiple ("| ", inIndentation) ;
  ioString << "|-" ;
  ioString << mFeatureList.reader_description  (_inLexique COMMA_THERE, inIndentation) ;
}
...
internalSubListWithRange (GGS_attributeInCollectionList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (_p) ;
      ioList._addAssign_operation (_p->mAttributeTypeName, _p->mAttributeName, _p->mFeatureList) ;
      _p = _p->nextObject () ;
    }
  }
}
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  ...
  if (equal) {
    const cPtr_listmapDeclaration * _p = dynamic_cast <const cPtr_listmapDeclaration *> (inOperand) ;
    macroValidPointer (_p) ;
    equal = mListmapTypeName._operator_isEqual (_p->mListmapTypeName).boolValue ()
         && mAssociatedListTypeName._operator_isEqual (_p->mAssociatedListTypeName).boolValue () ;
  }
  return equal ;
}
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString << "->@listmapDeclaration:"
           ...
           << mAssociatedListTypeName.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
internalSubListWithRange (GGS_mapMethodList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (_p) ;
      ioList._addAssign_operation (_p->mMethodName, _p->mErrorMessage, _p->mShadowErrorMessage) ;
      _p = _p->nextObject () ;
    }
  }
}
...
isEqualToObject (const cListElement * inOperand) const {
  bool equal = inOperand == this ;
  if (! equal) {
    const elementOf_GGS_lazyAttributeList * _p = dynamic_cast <const elementOf_GGS_lazyAttributeList *> (inOperand) ;
    macroValidPointer (_p) ;
...
internalSubListWithRange (GGS_lazyAttributeList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (_p) ;
      ioList._addAssign_operation (_p->mLocationMagicNumber, _p->mAttributeList, _p->mInstructionList) ;
      _p = _p->nextObject () ;
    }
  }
}
...
reader_description (C_Compiler & _inLexique
                    COMMA_LOCATION_ARGS,
                    const sint32 inIndentation) const {
  return _description (_inLexique, "@lazyAttributeList", inIndentation COMMA_THERE) ;
}
...
appendForMapDescription (C_Compiler & _inLexique,
  ...
  ioString << "|-value " ;
  ioString.appendSigned (inElementIndex) ;
  ioString << ":" << mInfo.mRoot.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
reader_description (C_Compiler & _inLexique
  ...
  if (isBuilt ()) {
    ...
    _s << mParsedSemanticsComponentMap.reader_description (_inLexique COMMA_THERE, inIndentation + 1) ;
  }else{
    _s << "not built" ;
  }
  _s << "\n" ;
