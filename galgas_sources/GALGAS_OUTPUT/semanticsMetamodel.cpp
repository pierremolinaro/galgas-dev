...
cPtr_categoryMethodDeclaration (const GGS_bool& argument_0,
                                const GGS_lstring & argument_1,
                                const GGS_lstring & argument_2,
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  ...
  if (equal) {
    ...
    macroValidPointer (_p) ;
    equal = mIsOverride._operator_isEqual (_p->mIsOverride).boolValue ()
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString << "->@categoryMethodDeclaration:"
           << mIsOverride.reader_description  (_inLexique COMMA_THERE, inIndentation + 1)
...
GGS_bool GGS_categoryMethodDeclaration::
reader_mIsOverride (C_Compiler & /* inLexique */ COMMA_UNUSED_LOCATION_ARGS) const {
  GGS_bool  result ;
  if (mPointer != NULL) {
    macroValidPointer (mPointer) ;
    MF_Assert (dynamic_cast <cPtr_categoryMethodDeclaration *> (mPointer) != NULL,
               "dynamic cast error", 0, 0) ;
    result = ((cPtr_categoryMethodDeclaration *) mPointer)->mIsOverride ;
  }
  return result ;
}
