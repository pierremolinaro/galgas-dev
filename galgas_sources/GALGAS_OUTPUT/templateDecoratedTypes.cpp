...
cPtr__AC_galgas_class * cPtr_templateInstructionForeachForGeneration::makeClone (void) const {
  cPtr__AC_galgas_class * result = NULL ;
  macroMyNew (result, cPtr_templateInstructionForeachForGeneration (mIsAscending, mExpression, mBeforeInstructionList, mDoInstructionList, mIndexIdentifier, mBetweenInstructionList, mAfterInstructionList COMMA_HERE)) ;
  return result ;
}
...
reader_mIndexIdentifier (C_Compiler & /* inLexique */ COMMA_UNUSED_LOCATION_ARGS) const {
  GGS_string  result ;
  if (mPointer != NULL) {
    macroValidPointer (mPointer) ;
    MF_Assert (dynamic_cast <cPtr_templateInstructionForeachForGeneration *> (mPointer) != NULL,
               "dynamic cast error", 0, 0) ;
    result = ((cPtr_templateInstructionForeachForGeneration *) mPointer)->mIndexIdentifier ;
  }
  return result ;
}
