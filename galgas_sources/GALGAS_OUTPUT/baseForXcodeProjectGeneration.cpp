...
void routine_generateXCodeFile (C_Compiler & _inLexique,
  ...
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (0U))).isBuiltAndTrue ()) {
    ...
    if ((var_cas_written).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_52102 = var_cas_fileNewContents ;
      if (_temp_52102.isBuilt ()) {
        _temp_52102 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, (var_cas_in_xcodeproj_filePath)._operator_concat (GGS_string ("/project.pbxproj")), automatic_var_0 COMMA_SOURCE_FILE_AT_LINE (1252)) ;
      }
    }
    if ((var_cas_written).isBuiltAndTrue ()) {
      ...
      {
        GGS_stringlist::cEnumerator enumerator_52402 (var_cas_pbxuser_files, true) ;
        const GGS_stringlist::cElement * operand_52402 = NULL ;
        while (((operand_52402 = enumerator_52402.nextObject ()))) {
          macroValidPointer (operand_52402) ;
          GGS_string var_cas_fullPath ;
          var_cas_fullPath = ((var_cas_in_xcodeproj_filePath)._operator_concat (GGS_string ("/")))._operator_concat (operand_52402->mValue) ;
