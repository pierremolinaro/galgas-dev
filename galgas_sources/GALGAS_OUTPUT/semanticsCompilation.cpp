...
void routine_compileSemanticsComponent (C_Compiler & inLexique,
  ...
  const GGS_bool cond_33296 = (GGS_uint ::constructor_errorCount (inLexique COMMA_HERE)).operator_isEqual (GGS_uint (0U)) ;
  if (cond_33296.isBuiltAndTrue ()) {
    ...
    {
      GGS_semanticDeclarationList::cEnumerator enumerator_32706 (var_cas_semanticDeclarationList, true) ;
      const GGS_semanticDeclarationList::cElement * operand_32706 = NULL ;
      while (((operand_32706 = enumerator_32706.nextObject ()))) {
        macroValidPointer (operand_32706) ;
        const GGS_semanticDeclaration  temp_32750 = operand_32706->mSemanticDeclaration ;
        if (temp_32750.isBuilt ()) {
          typeCategoryMethod__semanticDeclaration__generateCode method = findCategoryMethod__semanticDeclaration__generateCode (temp_32750.galgasObjectRunTimeInfo ()) ;
          if (method != NULL) {
            method (inLexique, temp_32750 (HERE), var_cas_semanticContext, var_cas_headerString, var_cas_implementationString COMMA_SOURCE_FILE_AT_LINE (872)) ;
          }
        }
      }
    }
