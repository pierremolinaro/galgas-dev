...
void routine_recursivelyImportSemanticComponent (C_Compiler & inLexique,
  ...
  #endif
  {
    GGS_lstringlist::cEnumerator enumerator_3165 (var_cas_inListOfFilesToParse, true) ;
    const GGS_lstringlist::cElement * operand_3165 = NULL ;
    while (((operand_3165 = enumerator_3165.nextObject ()))) {
      macroValidPointer (operand_3165) ;
      const GGS_bool cond_4093 = (var_cas_ioParsedFileSet.reader_hasKey (operand_3165->mValue.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (59)))).operator_not () ;
      if (cond_4093.isBuiltAndTrue ()) {
        var_cas_ioParsedFileSet.addAssign_operation (operand_3165->mValue.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (60))) ;
        GGS_string var_cas_extension = operand_3165->mValue.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (61)).reader_pathExtension () ;
        const GGS_bool cond_4080 = (var_cas_extension).operator_isEqual (GGS_string ("gSemantics")) ;
        if (cond_4080.isBuiltAndTrue ()) {
          GGS_lstring  var_cas_filePath = GGS_lstring ::constructor_new (inLexique, operand_3165->mValue.ggs_string ().reader_absolutePathFromPath (var_cas_inDirectory), operand_3165->mValue.reader_location (inLexique COMMA_SOURCE_FILE_AT_LINE (65)) COMMA_HERE) ;
...
void routine_compileSemanticsComponent (C_Compiler & inLexique,
  ...
  const GGS_bool cond_4983 = (var_cas_componentName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (103))).operator_isNotEqual (var_cas_basename) ;
