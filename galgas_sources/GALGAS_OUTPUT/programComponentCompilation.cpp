...
GALGAS_string filewrapperTemplate_programFileGenerationTemplate_programFileImplementation (C_Compiler * /* inCompiler */,
  ...
  result << ",\n"
    ...
    "        PMSInt16 r = 0 ;\n"
    "        " ;
  GALGAS_uint index_4958_IDX (0) ;
  if (in_PROGRAM_5F_RULE_5F_LIST.isValid ()) {
    cEnumerator_programRuleList enumerator_4958 (in_PROGRAM_5F_RULE_5F_LIST, kEnumeration_up) ;
    while (enumerator_4958.hasCurrentObject ()) {
      result << "if (fileExtension == " ;
      result << enumerator_4958.current_mSourceFileExtension (HERE).mAttribute_string.reader_utf_38_Representation (SOURCE_FILE_AT_LINE (121)).stringValue () ;
      result << ") {\n" ;
      const enumGalgasBool test_1 = in_HAS_5F_INDEXING.boolEnum () ;
      if (kBoolTrue == test_1) {
        const enumGalgasBool test_2 = GALGAS_bool (kIsEqual, enumerator_4958.current_mIndexingGrammar (HERE).reader_string (SOURCE_FILE_AT_LINE (123)).objectCompare (GALGAS_string::makeEmptyString ())).boolEnum () ;
        if (kBoolTrue == test_2) {
          result << "          if (! gOption_performIndexing.mValue) {\n"
            "            routine_" ;
          result << GALGAS_string ("programRule_").operator_concat (index_4958_IDX.reader_string (SOURCE_FILE_AT_LINE (125)) COMMA_SOURCE_FILE_AT_LINE (125)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (125)).stringValue () ;
          result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n"
            "          }\n" ;
        }else if (kBoolFalse == test_2) {
          result << "          if (gOption_performIndexing.mValue) {\n"
            "            cGrammar_" ;
          result << enumerator_4958.current_mIndexingGrammar (HERE).mAttribute_string.reader_identifierRepresentation (SOURCE_FILE_AT_LINE (129)).stringValue () ;
          result << "::performIndexing (commonLexique, sourceFilesArray (i COMMA_HERE)) ;\n"
            "          }else{\n"
            "            routine_" ;
          result << GALGAS_string ("programRule_").operator_concat (index_4958_IDX.reader_string (SOURCE_FILE_AT_LINE (131)) COMMA_SOURCE_FILE_AT_LINE (131)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (131)).stringValue () ;
          result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n"
            "          }\n" ;
        }
      }else if (kBoolFalse == test_1) {
        result << "          routine_" ;
        result << GALGAS_string ("programRule_").operator_concat (index_4958_IDX.reader_string (SOURCE_FILE_AT_LINE (135)) COMMA_SOURCE_FILE_AT_LINE (135)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (135)).stringValue () ;
        result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n" ;
      }
      if (enumerator_4958.hasNextObject ()) {
        result << "        }else " ;
      }
      index_4958_IDX.increment () ;
