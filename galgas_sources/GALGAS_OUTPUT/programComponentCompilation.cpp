...
//                    november 28th, 2010, at 17h25'24"                      *
//                                                                           *
//---------------------------------------------------------------------------*
...
GALGAS_string filewrapperTemplate_programFileGenerationTemplate_programFileImplementation (C_Compiler * /* inCompiler */,
  ...
  result << ",\n"
    ...
    "        PMSInt16 r = 0 ;\n"
    "        " ;
  GALGAS_uint index_4460_IDX (0) ;
  if (in_PROGRAM_5F_RULE_5F_LIST.isValid ()) {
    cEnumerator_programRuleList enumerator_4460 (in_PROGRAM_5F_RULE_5F_LIST, true) ;
    while (enumerator_4460.hasCurrentObject ()) {
      if (in_HAS_5F_INDEXING.isValidAndTrue ()) {
        result << "if (fileExtension == " ;
        result << enumerator_4460.current_mSourceFileExtension (HERE).mAttribute_string.reader_utf_38_Representation (SOURCE_FILE_AT_LINE (111)).stringValue () ;
        result << ") {\n" ;
        if (GALGAS_bool (kIsEqual, enumerator_4460.current_mIndexingGrammar (HERE).reader_string (SOURCE_FILE_AT_LINE (112)).objectCompare (GALGAS_string::makeEmptyString ())).isValidAndTrue ()) {
          result << "          if (! gOption_performIndexing.mValue) {\n"
            "            routine_" ;
          result << GALGAS_string ("programRule_").operator_concat (index_4460_IDX.reader_string (SOURCE_FILE_AT_LINE (114)) COMMA_SOURCE_FILE_AT_LINE (114)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (114)).stringValue () ;
          result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n"
            "          }\n" ;
        }else{
          result << "          if (gOption_performIndexing.mValue) {\n"
            "            cGrammar_" ;
          result << enumerator_4460.current_mIndexingGrammar (HERE).mAttribute_string.reader_identifierRepresentation (SOURCE_FILE_AT_LINE (118)).stringValue () ;
          result << "::performIndexing (commonLexique, sourceFilesArray (i COMMA_HERE)) ;\n"
            "          }else{\n"
            "            routine_" ;
          result << GALGAS_string ("programRule_").operator_concat (index_4460_IDX.reader_string (SOURCE_FILE_AT_LINE (120)) COMMA_SOURCE_FILE_AT_LINE (120)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (120)).stringValue () ;
          result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n"
            "          }\n" ;
        }
      }else{
        result << "          routine_" ;
        result << GALGAS_string ("programRule_").operator_concat (index_4460_IDX.reader_string (SOURCE_FILE_AT_LINE (124)) COMMA_SOURCE_FILE_AT_LINE (124)).reader_identifierRepresentation (SOURCE_FILE_AT_LINE (124)).stringValue () ;
        result << " (sourceFilePath, commonLexique COMMA_HERE) ;\n" ;
      }
      if (enumerator_4460.hasNextObject ()) {
        result << "        }else " ;
      }
      index_4460_IDX.increment () ;
