    ...
    if ((_mMatchedTemplateDelimiterIndex >= 0) && (UNICODE_ACCESS (mCurrentChar) != '\0')) {
      ...
      try{
        if (testForInputChar (UNICODE_NEW ('a'), UNICODE_NEW ('z')) ||
          ...
          do {
            if (testForInputChar (UNICODE_NEW (1), UNICODE_NEW ('\t')) ||
                testForInputChar (UNICODE_NEW ('\v')) ||
                testForInputChar (UNICODE_NEW ('\f')) ||
                testForInputChar (UNICODE_NEW (14), UNICODE_NEW (255))) {
            }else{
              loop_ = false ;
            }
          }while (loop_) ;
          loop_ = true ;
        }else if (testForInputChar (UNICODE_NEW (1), UNICODE_NEW (' '))) {
