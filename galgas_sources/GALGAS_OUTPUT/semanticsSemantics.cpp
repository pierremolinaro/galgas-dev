...
method_checkNoCircularity (C_Compiler & _inLexique,
  ...
  GGS_uint _variant_26979 = var_cas_inTypeCount ;
  GGS_bool _condition_26979 (_variant_26979._isBuilt (), true) ;
  while (_condition_26979.isBuiltAndTrue ()) {
    _condition_26979 = ((var_cas_superClassIndex.reader_isRegular (_inLexique COMMA_SOURCE_FILE_AT_LINE (508)))._operator_and (var_cas_ok))._operator_and ((var_cas_circularityError)._operator_not ()) ;
    if (_condition_26979.isBuiltAndTrue ()) {
      if (_variant_26979.uintValue () == 0) {
        _inLexique.onTheFlyRunTimeError ("loop variant error" COMMA_SOURCE_FILE_AT_LINE (536)) ;
        _condition_26979 = GGS_bool (true, false) ;
      }else{
        _variant_26979._decrement_operation (_inLexique COMMA_HERE) ;
        if ((var_cas_superClassIndex.reader_isSolved (_inLexique COMMA_SOURCE_FILE_AT_LINE (509))).isBuiltAndTrue ()) {
          GGS_lstring  var_cas_superClassName ;
          GGS_ACGalgasType  var_cas_t ;
          const GGS_typeMapIndex  _temp_25944 = var_cas_superClassIndex ;
          if (_temp_25944._isBuilt ()) {
            _temp_25944 (HERE)->method_searchKey (_inLexique, var_cas_superClassName, var_cas_t COMMA_SOURCE_FILE_AT_LINE (513)) ;
          }
          var_cas_circularityError = var_cas_superClassSet.reader_hasKey (_inLexique, var_cas_superClassName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (515)) COMMA_SOURCE_FILE_AT_LINE (515)) ;
          var_cas_superClassSet._addAssign_operation (var_cas_superClassName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (516))) ;
          var_cas_classPath.writeString (".") ;
          var_cas_classPath._dotAssign_operation (var_cas_superClassName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (517))) ;
          GGS_typeMapIndex  var_cas_superSuperClassIndex ;
          { const GGS_ACGalgasType _var_26654 = var_cas_t ; // CAST instruction
            if (_var_26654.getPtr () != NULL) {
              macroValidPointer (_var_26654.getPtr ()) ;
              if (dynamic_cast <cPtr_classGalgasType *> (_var_26654.getPtr ()) != NULL) {
                const GGS_classGalgasType var_cas_kClassType (_var_26654.getPtr ()) ;
                var_cas_superSuperClassIndex = var_cas_kClassType.reader_mSuperClassIndex (_inLexique COMMA_SOURCE_FILE_AT_LINE (522)) ;
              }else{
          ...
          var_cas_superClassName = var_cas_superClassIndex.reader_key (_inLexique COMMA_SOURCE_FILE_AT_LINE (532)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (532)) ;
          mClassName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "this class inherits from @"))._operator_concat (var_cas_superClassName))._operator_concat (GGS_string (true, ", but @")))._operator_concat (var_cas_superClassName))._operator_concat (GGS_string (true, " does not exist")) COMMA_SOURCE_FILE_AT_LINE (535)) ;
        }
      }
    }
  }
  if ((var_cas_circularityError).isBuiltAndTrue ()) {
    mClassName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, (((GGS_string (true, "inheritance circularity for class '"))._operator_concat (mClassName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (542))))._operator_concat (GGS_string (true, "': ")))._operator_concat (var_cas_classPath.reader_stringByReplacingStringByString (_inLexique, GGS_string (true, "."), GGS_string (true, " extends ") COMMA_SOURCE_FILE_AT_LINE (543))) COMMA_SOURCE_FILE_AT_LINE (543)) ;
  }
}
...
method_checkNoCircularity (C_Compiler & _inLexique,
  ...
  GGS_ACGalgasType  var_cas_t ;
  const GGS_typeMapIndex  _temp_30134 = mIndexOfMapType ;
  if (_temp_30134._isBuilt ()) {
    _temp_30134 (HERE)->method_searchKey (_inLexique, var_cas_mapTypeName, var_cas_t COMMA_SOURCE_FILE_AT_LINE (614)) ;
  }
  { const GGS_ACGalgasType _var_30389 = var_cas_t ; // CAST instruction
    if (_var_30389.getPtr () != NULL) {
      macroValidPointer (_var_30389.getPtr ()) ;
      if (dynamic_cast <cPtr_mapType *> (_var_30389.getPtr ()) != NULL) {
      }else{
        var_cas_mapTypeName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "tis mapindex type refers to @"))._operator_concat (var_cas_mapTypeName))._operator_concat (GGS_string (true, " as a map, but @")))._operator_concat (var_cas_mapTypeName))._operator_concat (GGS_string (true, " is not a map")) COMMA_SOURCE_FILE_AT_LINE (621)) ;
      }
    }
  }
}
