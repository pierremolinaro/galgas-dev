...
GGS_typeList::GGS_typeList (const bool inBuildObject) :
AC_galgas_list () {
  if (inBuildObject) {
    _alloc () ;
  }
...
GGS_signature::GGS_signature (const bool inBuildObject) :
AC_galgas_list () {
  if (inBuildObject) {
    _alloc () ;
  }
...
GGS_attributeList::GGS_attributeList (const bool inBuildObject) :
AC_galgas_list () {
  if (inBuildObject) {
    _alloc () ;
  }
...
GGS_classMethodList::GGS_classMethodList (const bool inBuildObject) :
AC_galgas_list () {
  if (inBuildObject) {
    _alloc () ;
  }
...
GGS_sortDescriptorList::GGS_sortDescriptorList (const bool inBuildObject) :
AC_galgas_list () {
  if (inBuildObject) {
    _alloc () ;
  }
...
method_checkNoCircularity (C_Compiler & _inLexique,
  ...
  GGS_ACGalgasType  var_cas_t ;
  const GGS_typeMapIndex  _temp_29797 = mIndexOfMapType ;
  if (_temp_29797._isBuilt ()) {
    _temp_29797 (HERE)->method_searchKey (_inLexique, var_cas_mapTypeName, var_cas_t COMMA_SOURCE_FILE_AT_LINE (606)) ;
  }
  { const GGS_ACGalgasType _var_30052 = var_cas_t ; // CAST instruction
    if (_var_30052.getPtr () != NULL) {
      macroValidPointer (_var_30052.getPtr ()) ;
      if (dynamic_cast <cPtr_mapType *> (_var_30052.getPtr ()) != NULL) {
      }else{
