...
method_checkNoCircularity (C_Compiler & _inLexique,
  ...
  GGS_uint _variant_26642 = var_cas_inTypeCount ;
  GGS_bool _condition_26642 (_variant_26642._isBuilt (), true) ;
  while (_condition_26642.isBuiltAndTrue ()) {
    _condition_26642 = ((var_cas_superClassIndex.reader_isRegular (_inLexique COMMA_SOURCE_FILE_AT_LINE (508)))._operator_and (var_cas_ok))._operator_and ((var_cas_circularityError)._operator_not ()) ;
    if (_condition_26642.isBuiltAndTrue ()) {
      if (_variant_26642.uintValue () == 0) {
        _inLexique.onTheFlyRunTimeError ("loop variant error" COMMA_SOURCE_FILE_AT_LINE (528)) ;
        _condition_26642 = GGS_bool (true, false) ;
      }else{
        _variant_26642._decrement_operation (_inLexique COMMA_HERE) ;
        GGS_lstring  var_cas_superClassName ;
        GGS_ACGalgasType  var_cas_t ;
        const GGS_typeMapIndex  _temp_25895 = var_cas_superClassIndex ;
        if (_temp_25895._isBuilt ()) {
          _temp_25895 (HERE)->method_searchKey (_inLexique, var_cas_superClassName, var_cas_t COMMA_SOURCE_FILE_AT_LINE (512)) ;
        }
        ...
        { const GGS_ACGalgasType _var_26577 = var_cas_t ; // CAST instruction
          if (_var_26577.getPtr () != NULL) {
            macroValidPointer (_var_26577.getPtr ()) ;
            if (dynamic_cast <cPtr_classGalgasType *> (_var_26577.getPtr ()) != NULL) {
              const GGS_classGalgasType var_cas_kClassType (_var_26577.getPtr ()) ;
...
method_checkNoCircularity (C_Compiler & _inLexique,
  ...
  GGS_ACGalgasType  var_cas_t ;
  const GGS_typeMapIndex  _temp_30313 = mIndexOfMapType ;
  if (_temp_30313._isBuilt ()) {
    _temp_30313 (HERE)->method_searchKey (_inLexique, var_cas_mapTypeName, var_cas_t COMMA_SOURCE_FILE_AT_LINE (617)) ;
  }
  { const GGS_ACGalgasType _var_30568 = var_cas_t ; // CAST instruction
    if (_var_30568.getPtr () != NULL) {
      macroValidPointer (_var_30568.getPtr ()) ;
      if (dynamic_cast <cPtr_mapType *> (_var_30568.getPtr ()) != NULL) {
      }else{
