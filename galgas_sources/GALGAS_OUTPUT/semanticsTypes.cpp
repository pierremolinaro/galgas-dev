...
internalSubListWithRange (GGS_typedAttributeList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (PMSInt32 i=0 ; i<inCount ; i++) {
      macroValidPointer (ptr) ;
      ioList.addAssign_operation (ptr->mEnumeratedType, ptr->mAttributeName) ;
      ptr = ptr->nextObject () ;
    }
  }
}
...
/* const */ GGS_unifiedTypeMapIndex  & GGS_typedAttributeList::cEnumerator::_mEnumeratedType (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
  return ((/* const */ cElement *) mCurrentEnumeratedObject)->mEnumeratedType ;
}
...
insertElement (C_Compiler & inLexique,
                ...
                const GGS_enumerationDescriptorList & inParameter8,
                GGS_luint * outIndex
                COMMA_LOCATION_ARGS) {
  PMSInt32 elementID = - 1 ;
  if (isBuilt ()
   ...
   && inKey.isBuilt ()) {
    ...
    info.mEnumerationDescriptor = inParameter8 ;
    GGS_location existingKeyLocation ;
    internalInsert (inKey, (void *) & info, elementID, existingKeyLocation) ;
    if (elementID < 0) {
      emitInsertMapSemanticErrorMessage (inLexique, inKey, inErrorMessage, existingKeyLocation COMMA_THERE) ;
...
searchElement (C_Compiler & inLexique,
               ...
               GGS_enumerationDescriptorList   & outParameter8,
               GGS_luint * outIndex
               COMMA_LOCATION_ARGS) const {
  cElement * node = NULL  ;
  if (isBuilt () && inKey.isBuilt ()) {
  ...
  if (node == NULL) {
    ...
    outParameter8 = node->mInfo.mEnumerationDescriptor ;
    if (outIndex != NULL) {
      * outIndex = GGS_luint (GGS_uint (true, (PMUInt32) node->mID), inKey) ;
    }
  }
...
method_searchKey (C_Compiler & inLexique,
                                ...
                                GGS_instanceMethodMap   & outParameter6,
                                GGS_classMethodMap   & outParameter7,
                                GGS_enumerationDescriptorList   & outParameter8 COMMA_LOCATION_ARGS) const {
  searchElement (inLexique,
                  kSearchMessage_searchKey,
                  inKey,
                  outParameter0,
...
modifier_insertKey (C_Compiler & inLexique,
                                ...
                                const GGS_instanceMethodMap & inParameter6,
                                const GGS_classMethodMap & inParameter7,
                                const GGS_enumerationDescriptorList & inParameter8 COMMA_LOCATION_ARGS) {
  insertElement (inLexique,
                 kInsertMessage_insertKey,
                 inKey,
                 inParameter0,
