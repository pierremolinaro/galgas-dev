...
pr_treewalking_routine_treewalking_parser_374_6_ (galgas_scanner & _inLexique,
  ...
  const GGS_entityPropertyMap _var_14964 = var_cas_allMetamodelPropertyMap ;
  GGS_entityPropertyMap::element_type * operand_14964 = _var_14964.firstObject () ;
  while ((operand_14964 != NULL)) {
    macroValidPointer (operand_14964) ;
    if (operand_14964->mInfo.mPropertyType._isBuilt ()) {
      cPtr_typeGalgas_singleReferenceEntity * operand_15056 = dynamic_cast <cPtr_typeGalgas_singleReferenceEntity *> (operand_14964->mInfo.mPropertyType.getPtr ()) ;
      if (operand_15056 != NULL) {
        macroValidPointer (operand_15056) ; 
      }else{
        cPtr_typeGalgas_multipleReferenceEntity * operand_15107 = dynamic_cast <cPtr_typeGalgas_multipleReferenceEntity *> (operand_14964->mInfo.mPropertyType.getPtr ()) ;
        if (operand_15107 != NULL) {
          macroValidPointer (operand_15107) ; 
        }else{ // Else part
          GGS_typeCplusPlusName  var_cas_cppName ;
          var_cas_cppName = GGS_typeCurrentObjectName::constructor_new (_inLexique, operand_14964->mKey.reader_location (_inLexique COMMA_SOURCE_FILE_AT_LINE (407)), operand_14964->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (407)) COMMA_HERE) ;
          var_cas_localVariableMap.modifier_insertUsedConstInArgument (_inLexique, operand_14964->mKey, operand_14964->mInfo.mPropertyType, var_cas_cppName COMMA_SOURCE_FILE_AT_LINE (408)) ;
        }
      }
    }
    operand_14964 = operand_14964->nextObject () ;
  }
  _inLexique.acceptTerminal (ACCEPT_TERMINAL (galgas_scanner::galgas_scanner_1__3A) COMMA_HERE) ;
