...
//---------------------------------------------------------------------------*
  ...
  static const char gGGSsourceFile [] = "galgas_LL1_prgm.ggs" ;
...
grammar_galgas_LL1_prgm::
grammar_galgas_LL1_prgm (const C_galgas_io_parameters & inParameters,
                  const C_galgas_io::outputKindEnum inOutputKindEnum
                  COMMA_LOCATION_ARGS) :
_mScannerPtr (NULL) {
  macroMyNew (_mScannerPtr, galgas_scanner (inParameters, inOutputKindEnum COMMA_THERE)) ;
}
  ...
  //--- Enable 64 bit alloc debug ? Only if compiled in 64 bit and in debug mode
    ...
    try{
      verboseOptionOn = options.boolOptionValueFromKeys ("generic_galgas_cli_options",
                                                         "verbose_output",
                                                         false) ;
      galgas_LL1_prgm_prologue (options, sourceFilesArray) ;
      for (sint32 i=0 ; i<sourceFilesArray.count () ; i++) {
        const C_String fileExtension = sourceFilesArray (i COMMA_HERE).pathExtension () ;
        sint16 r = 0 ;
        if (fileExtension.compare ("ggs") == 0) {
          grammar_galgas_LL1_prgm * compiler = NULL ;
          macroMyNew (compiler, grammar_galgas_LL1_prgm (IOparameters, C_galgas_io::kTerminalOutputKind COMMA_HERE)) ;
          compiler->_prologue () ;
          compiler->doCompilation (sourceFilesArray (i COMMA_HERE), verboseOptionOn, r) ;
          compiler->_epilogue () ;
          macroMyDelete (compiler, grammar_galgas_LL1_prgm) ;
        }else{
      ...
      galgas_LL1_prgm_epilogue (options) ;
...
void
galgas_LL1_prgm_prologue (const C_options_for_galgas_LL1_prgm & inOptions,
...
void grammar_galgas_LL1_prgm::_beforeParsing (void) {
  ...
  _mScannerPtr->mFileGenerationStartDir = _mScannerPtr->sourceFileName ().stringByDeletingLastPathComponent ().stringByAppendingPathComponent ("GALGAS_OUTPUT") ;
  const bool ok = _mScannerPtr->mFileGenerationStartDir.makeDirectoryIfDoesNotExists () ;
  if (! ok) {
    C_String errorMessage ;
    errorMessage << "cannot create directory '" << _mScannerPtr->mFileGenerationStartDir << "'" ;
    _mScannerPtr->printFileErrorMessage (_mScannerPtr->sourceFileName (), errorMessage.cString () COMMA_HERE) ;
  }
}
...
void grammar_galgas_LL1_prgm::_afterParsing (const bool inVerboseOptionOn) {
  if (_mScannerPtr->boolOptionValueFromKeys ("galgas_cli_options", "warningsAsError" COMMA_HERE) &&
