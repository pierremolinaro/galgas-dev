...
appendForDescription (C_Compiler & inLexique,
  ...
  ioString.writeStringMultiple ("| ", inIndentation) ;
  ioString << "|-" ;
  ioString << mTerminalList.reader_description  (inLexique COMMA_THERE, inIndentation) ;
}
...
internalSubListWithRange (GGS_guiLabelList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (ptr) ;
      ioList._addAssign_operation (ptr->mTerminalList) ;
      ptr = ptr->nextObject () ;
    }
  }
}
...
/* const */ GGS_lstringlist  & GGS_guiLabelList::cEnumerator::_mTerminalList (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
  return ((cElement *) mCurrentEnumeratedObject)->mTerminalList ;
}
