...
_addAssign_operation (const GGS_appendInstruction & inElement) {
  if (_isBuilt () && inElement._isBuilt ()) {
    _insulateList () ;
    _internalAppendValues (((cPtr_appendInstruction *) inElement.getPtr ())->mReceiverName,
                                ((cPtr_appendInstruction *) inElement.getPtr ())->mExpressionList) ;
  }
}
...
_cloneObject (void) const {
  ...
  macroMyNew (_p, cPtr_grammarInstruction (mGrammarComponentName, mLabelName, mSourceFileExpression, mActualParameterList COMMA_HERE)) ;
  return _p ;
}
