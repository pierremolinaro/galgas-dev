...
elementOf_GGS_switchBranches (const GGS_lstring & argument_0,
                                const GGS_semanticInstructionList & argument_1
                                COMMA_LOCATION_ARGS) :
AC_galgas_list::cListElement (THERE),
mSwitchConstant (argument_0),
...
isEqualToObject (const cListElement * inOperand) const {
  ...
  if (! equal) {
    ...
    macroValidPointer (_p) ;
    equal = mSwitchConstant._operator_isEqual (_p->mSwitchConstant).boolValue ()
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString.writeStringMultiple ("| ", inIndentation) ;
  ioString << "|-" ;
  ioString << mSwitchConstant.reader_description  (_inLexique COMMA_THERE, inIndentation) ;
  ioString << "\n" ;
  ioString.writeStringMultiple ("| ", inIndentation) ;
  ioString << "|-" ;
    ...
    if (count () == 0) {
      * this = inOperand ;
      ...
      if (inOperand.count () > 0) {
        ...
        while (p != NULL) {
          macroValidPointer (p) ;
          GGS_lstring  p_0 = p->mSwitchConstant ;
...
_insulateList (void) {
  if (_shared ()) {
    ...
    while (_p != NULL) {
      macroValidPointer (_p) ;
      _internalAppendValues (_p->mSwitchConstant,
                                _p->mInstructions
                                COMMA_HERE) ;
      _p = _p->nextObject () ;
    }
...
internalSubListWithRange (GGS_switchBranches & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (_p) ;
      ioList._addAssign_operation (_p->mSwitchConstant, _p->mInstructions) ;
      _p = _p->nextObject () ;
    }
  }
}
...
GGS_lstring  GGS_switchBranches::
reader_mSwitchConstantAtIndex (C_Compiler & inLexique, const GGS_uint & inIndex COMMA_LOCATION_ARGS) const {
  GGS_lstring  result ;
  if (_isBuilt () && inIndex._isBuilt ()) {
    cElement * object = (cElement *) objectAtIndex (inLexique, inIndex.uintValue () COMMA_THERE) ;
    if (object != NULL) {
      result = object->mSwitchConstant ;
    }
  }
  return result ;
}
...
void GGS_switchBranches::
modifier_setMSwitchConstantAtIndex (C_Compiler & inLexique,
                              const GGS_lstring  & inObject,
                              const GGS_uint & inIndex
                              COMMA_LOCATION_ARGS) const {
  if (_isBuilt () && inIndex._isBuilt ()) {
    cElement * object = (cElement *) objectAtIndex (inLexique, inIndex.uintValue () COMMA_THERE) ;
    if (object != NULL) {
      object->mSwitchConstant = inObject ;
    }
  }
}
...
/* const */ GGS_lstring  & GGS_switchBranches::cEnumerator::_mSwitchConstant (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
  return ((cElement *) mCurrentEnumeratedObject)->mSwitchConstant ;
}
...
/* const */ GGS_formalParameterList  & GGS_abstractCategoryMethodList::cEnumerator::_mFormalParameterList (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
  return ((cElement *) mCurrentEnumeratedObject)->mFormalParameterList ;
}
