...
pr_galgas_component_program_parser_111_6_ (galgas_scanner & _inLexique,
  ...
  const GGS_stringset _var_6717 = var_cas_includedSemanticsComponents ;
  const GGS_stringset::element_type * operand_6717 = _var_6717.firstObject () ;
  while ((operand_6717 != NULL)) {
    macroValidPointer (operand_6717) ;
    var_cas_inclusionsForImplementationFile._addAssign_operation ((operand_6717->mKey)._operator_concat (GGS_string (true, ".h"))) ;
    operand_6717 = operand_6717->nextObject () ;
  }
  ...
  { bool syntaxRepeat = true ;
    while (syntaxRepeat) {
        ...
        if (((var_cas_sourceExtension.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (186)).reader_length (_inLexique COMMA_SOURCE_FILE_AT_LINE (186)))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
          ...
          GGS_uint _variant_7933 = var_cas_sourceExtension.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (191)).reader_length (_inLexique COMMA_SOURCE_FILE_AT_LINE (191))._add_operation (_inLexique, GGS_uint (true, 1U) COMMA_SOURCE_FILE_AT_LINE (191)) ;
          GGS_bool _condition_7933 (_variant_7933._isBuilt (), true) ;
          while (_condition_7933.isBuiltAndTrue ()) {
            _condition_7933 = ((var_cas_index)._operator_strictInf (var_cas_sourceExtension.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (192)).reader_length (_inLexique COMMA_SOURCE_FILE_AT_LINE (192))))._operator_and (var_cas_ok) ;
            if (_condition_7933.isBuiltAndTrue ()) {
              if (_variant_7933.uintValue () == 0) {
                _inLexique.onTheFlyRunTimeError ("loop variant error" COMMA_SOURCE_FILE_AT_LINE (196)) ;
                _condition_7933 = GGS_bool (true, false) ;
              }else{
                _variant_7933._decrement_operation (_inLexique COMMA_HERE) ;
                GGS_char var_cas_c ;
