    ...
    while (syntaxRepeat_2) {
      switch (select_repeat_programModelBuilder_2 (_inLexique)) {
        case 2 : {
          ...
          if (((var_cas_typeName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (96)))._operator_isNotEqual (GGS_string (true, "lstring"))).isBuiltAndTrue ()) {
            var_cas_typeName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "the only type allowed here is '@lstring'") COMMA_SOURCE_FILE_AT_LINE (97)) ;
          }
