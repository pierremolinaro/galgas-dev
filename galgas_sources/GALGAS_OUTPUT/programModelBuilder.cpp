    ...
    while (syntaxRepeat_2) {
      switch (select_programModelBuilder_2 (inLexique)) {
        case 2 : {
          ...
          const GGS_bool cond_4494 = (var_cas_typeName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (118))).operator_isNotEqual (GGS_string ("lstring")) ;
          if (cond_4494.isBuiltAndTrue ()) {
            var_cas_typeName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("the only type allowed here is '@lstring'") COMMA_SOURCE_FILE_AT_LINE (120)) ;
          }
