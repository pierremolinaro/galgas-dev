...
isEqualToObject (const cListElement * inOperand) const {
  ...
  if (! equal) {
    ...
    equal = mTerminalName.operator_isEqual (ptr->mTerminalName).boolValue ()
         && mArgumentTypeList.operator_isEqual (ptr->mArgumentTypeList).boolValue () ;
  }
  return equal ;
}
...
appendForDescription (C_Compiler & inLexique,
  ...
  ioString.writeStringMultiple ("| ", inIndentation) ;
  ioString << "|-" ;
  ioString << mArgumentTypeList.reader_description  (inLexique COMMA_THERE, inIndentation) ;
}
      ...
      * this = inOperand ;
      ...
      if (inOperand.count () > 0) {
        ...
        while (p != NULL) {
          ...
          GGS_lexicalTypeList  p_1 = p->mArgumentTypeList ;
...
internalSubListWithRange (GGS_terminalList & ioList,
  ...
  if (inCount > 0) {
    ...
    for (sint32 i=0 ; i<inCount ; i++) {
      macroValidPointer (ptr) ;
      ioList._addAssign_operation (ptr->mTerminalName, ptr->mArgumentTypeList) ;
      ptr = ptr->nextObject () ;
    }
  }
}
...
modifier_setMArgumentTypeListAtIndex (C_Compiler & inLexique,
  ...
  if (isBuilt () && inIndex.isBuilt ()) {
    cElement * object = (cElement *) objectAtIndex (inLexique, inIndex.uintValue () COMMA_THERE) ;
    if (object != NULL) {
      object->mArgumentTypeList = inObject ;
    }
  }
}
...
/* const */ GGS_lexicalTypeList  & GGS_terminalList::cEnumerator::_mArgumentTypeList (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
  return ((cElement *) mCurrentEnumeratedObject)->mArgumentTypeList ;
}
