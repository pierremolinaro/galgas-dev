...
modifier_insertKey (C_Compiler & inLexique,
  ...
  _insertElement (inLexique,
                 "the '%K' terminal has been already declared in the list",
                 inKey,
                 inParameter0,
                 inParameter1,
                 NULL
...
method_searchKey (C_Compiler & inLexique,
  ...
  _searchElement (inLexique,
                  "the '%K' token list is not declared",
                  inKey,
                  outParameter0,
                  NULL
                  COMMA_THERE) ;
...
modifier_insertKey (C_Compiler & inLexique,
  ...
  _insertElement (inLexique,
                 "the '%K' token list has been already declared in %L",
                 inKey,
                 inParameter0,
                 NULL
                 COMMA_THERE) ;
...
appendForMapDescription (C_Compiler & inLexique,
  ...
  ioString << "|-value " ;
  ioString.appendSigned (inElementIndex) ;
  ioString << ":" << mInfo.messageIsUsed.reader_description  (inLexique COMMA_THERE, inIndentation + 1) ;
}
...
isEqualToMapElement (const AC_galgas_map_element * inOperand) const {
  ...
  return (mInfo.mLexicalMessage._operator_isEqual (ptr->mInfo.mLexicalMessage)).boolValue ()
           && (mInfo.messageIsUsed._operator_isEqual (ptr->mInfo.messageIsUsed)).boolValue () ;
}
...
_insertElement (C_Compiler & inLexique,
  ...
  if (isBuilt ()
   ...
   && inKey.isBuilt ()) {
    ...
    info.messageIsUsed = inParameter1 ;
    GGS_location existingKeyLocation ;
    internalInsert (inKey, (void *) & info, elementID, existingKeyLocation) ;
    if (elementID < 0) {
      emitInsertMapSemanticErrorMessage (inLexique, inKey, inErrorMessage, existingKeyLocation COMMA_THERE) ;
...
_searchElement (C_Compiler & inLexique,
  ...
  if (node == NULL) {
    ...
    outParameter1 = node->mInfo.messageIsUsed ;
    if (outIndex != NULL) {
      * outIndex = GGS_luint (GGS_uint (true, (uint32) node->mID), inKey) ;
    }
  }
...
void GGS_lexicalMessageMap::
modifier_setMessageIsUsedForKey (C_Compiler & inLexique,
  ...
  if (isBuilt () && inValue.isBuilt () && inKey.isBuilt ()) {
    ...
    if (node == NULL) {
      ...
      errorMessage << "the '" << inKey << "' key does not exist when calling 'setMessageIsUsedForKey' modifier" ;
      inLexique.onTheFlyRunTimeError (errorMessage COMMA_THERE) ;
    }else{
      node->mInfo.messageIsUsed = inValue ;
    }
  }
}
...
/* const */ GGS_bool & GGS_lexicalMessageMap::cEnumerator::_messageIsUsed (LOCATION_ARGS) const {
  return ((cElement *) mObjectArray (mCurrentIndex COMMA_THERE))->mInfo.messageIsUsed ;
}
...
bool GGS_lexiqueAnalysisContext::isBuilt (void) const {
  return mLexicalMessageMap.isBuilt ()
...
_operator_isEqual (const GGS_lexiqueAnalysisContext & inOperand) const {
  return mLexicalMessageMap._operator_isEqual (inOperand.mLexicalMessageMap)
...
_operator_isNotEqual (const GGS_lexiqueAnalysisContext & inOperand) const {
  return mLexicalMessageMap._operator_isNotEqual (inOperand.mLexicalMessageMap)
...
constructor_new (const GGS_lexicalMessageMap & argument_0,
                 const GGS_terminalMap & argument_1,
                 const GGS_lexicalAttributeMap & argument_2,
                 const GGS_lexicalExplicitTokenListMapMap & argument_3) {
  GGS_lexiqueAnalysisContext result ;
  result.mLexicalMessageMap = argument_0 ;
  result.mTerminalMap = argument_1 ;
  result.mLexicalAttributeMap = argument_2 ;
  result.mLexicalTokenListMapMap = argument_3 ;
  return result ;
}
