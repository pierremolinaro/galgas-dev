...
GALGAS_string function_outputDirectory (const GALGAS_lstring & constinArgument_inSourceFile,
  ...
  const enumGalgasBool test_0 = GALGAS_bool (kIsEqual, constinArgument_inSourceFile.mAttribute_string.reader_characterAtIndex (GALGAS_uint ((uint32_t) 0U), inCompiler COMMA_SOURCE_FILE_AT_LINE (367)).objectCompare (GALGAS_char (TO_UNICODE (47)))).boolEnum () ;
  if (kBoolTrue == test_0) {
...
void routine_performProjectGlobalCheckings (const GALGAS_parsedComponentStruct constinArgument_inParsedComponentStruct,
  ...
  const enumGalgasBool test_1 = GALGAS_bool (kIsEqual, GALGAS_uint::constructor_errorCount (SOURCE_FILE_AT_LINE (554)).objectCompare (GALGAS_uint ((uint32_t) 0U))).boolEnum () ;
  if (kBoolTrue == test_1) {
  ...
  if (kBoolTrue == test_2) {
    ...
    while (enumerator_34328.hasCurrentObject ()) {
      var_firstLetterSet.addAssign_operation (enumerator_34328.current_lkey (HERE).mAttribute_string.reader_characterAtIndex (GALGAS_uint ((uint32_t) 0U), inCompiler COMMA_SOURCE_FILE_AT_LINE (719)).reader_string (SOURCE_FILE_AT_LINE (719)) COMMA_SOURCE_FILE_AT_LINE (719)) ;
    ...
    while (enumerator_34533.hasCurrentObject ()) {
      const enumGalgasBool test_3 = GALGAS_bool (kIsNotEqual, var_currentFirstLetter.objectCompare (enumerator_34533.current_lkey (HERE).mAttribute_string.reader_characterAtIndex (GALGAS_uint ((uint32_t) 0U), inCompiler COMMA_SOURCE_FILE_AT_LINE (724)))).boolEnum () ;
      if (kBoolTrue == test_3) {
        var_currentFirstLetter = enumerator_34533.current_lkey (HERE).mAttribute_string.reader_characterAtIndex (GALGAS_uint ((uint32_t) 0U), inCompiler COMMA_SOURCE_FILE_AT_LINE (725)) ;
