...
void routine_compileSemanticsComponentLL1 (C_Compiler & _inLexique,
  ...
  _inLexique.printMessage (C_String ("LOGGING routineMap: ") + var_cas_routineMap.reader_description (_inLexique COMMA_HERE) + "\n" COMMA_SOURCE_FILE_AT_LINE (147)) ;
  if (((var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (149)))._operator_strictSup (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    GGS_string var_cas_m ;
    var_cas_m = (var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (150)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (150)))._operator_concat (GGS_string (true, " type")) ;
    if (((var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (151)))._operator_isEqual (GGS_uint (true, 1U))).isBuiltAndTrue ()) {
      var_cas_m = GGS_string (true, "1 type is not defined:") ;
    }else{
      var_cas_m = (var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (154)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (154)))._operator_concat (GGS_string (true, " types are not defined:")) ;
    }
    const GGS_stringlist _var_7064 = var_cas_typeMap.reader_unsolvedIndexKeyStringList (_inLexique COMMA_SOURCE_FILE_AT_LINE (156)) ;
    const GGS_stringlist::element_type * operand_7064 = _var_7064.firstObject () ;
    while ((operand_7064 != NULL)) {
      macroValidPointer (operand_7064) ;
      var_cas_m.writeString ("\n  - @") ;
      var_cas_m._dotAssign_operation (operand_7064->mValue) ;
      operand_7064 = operand_7064->nextObject () ;
    }
    var_cas_endOfSourceFile.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (160)) ;
  }
  const GGS_typeMap _var_7268 = var_cas_typeMap ;
  const GGS_typeMap::element_type * operand_7268 = _var_7268.firstObject () ;
  while ((operand_7268 != NULL)) {
    macroValidPointer (operand_7268) ;
    if (operand_7268->mInfo.mType._isBuilt ()) {
      operand_7268->mInfo.mType (HERE)->method_checkNoCircularity (_inLexique, var_cas_typeMap.reader_count (_inLexique COMMA_SOURCE_FILE_AT_LINE (163)) COMMA_SOURCE_FILE_AT_LINE (163)) ;
    }
    operand_7268 = operand_7268->nextObject () ;
  }
  #ifdef DEBUG_TRACE_ENABLED
  ...
  { galgas_scanner * scanner_ = NULL ;
    ...
    if (sourceFileName.fileExists ()) {
      ...
      if (scanner_->sourceText () != NULL) {
        ...
        C_String message ;
        message << "the '" << sourceFileName << "' file exits, but cannot be read" ;
        var_cas_inSourceFile.signalSemanticError (_inLexique, message COMMA_SOURCE_FILE_AT_LINE (174)) ;
      }
      macroDetachPointer (scanner_, galgas_scanner) ;
    }else{
      C_String message ;
  ...
  { galgas_scanner * scanner_ = NULL ;
    ...
    if (sourceFileName.fileExists ()) {
      ...
      if (scanner_->sourceText () != NULL) {
        ...
        C_String message ;
        message << "the '" << sourceFileName << "' file exits, but cannot be read" ;
        var_cas_inSourceFile.signalSemanticError (_inLexique, message COMMA_SOURCE_FILE_AT_LINE (184)) ;
      }
      macroDetachPointer (scanner_, galgas_scanner) ;
    }else{
      C_String message ;
  ...
  { galgas_scanner * scanner_ = NULL ;
    ...
    if (sourceFileName.fileExists ()) {
      ...
      if (scanner_->sourceText () != NULL) {
        ...
        C_String message ;
        message << "the '" << sourceFileName << "' file exits, but cannot be read" ;
        var_cas_inSourceFile.signalSemanticError (_inLexique, message COMMA_SOURCE_FILE_AT_LINE (194)) ;
      }
      macroDetachPointer (scanner_, galgas_scanner) ;
    }else{
      C_String message ;
