...
void routine_compileMetamodelComponentLL1 (C_Compiler & _inLexique,
  ...
  if (((GGS_uint::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    var_cas_generatedCode3.writeString ("sint32 _metamodel_index_for_expressionMetamodel (void) {\n"
    "  return gMetamodelManager.mMetamodelComponentIndex ;\n"
    "}\n\n") ;
    const GGS_sortedTypeList _var_10297 = var_cas_sortedTypeList ;
    const GGS_sortedTypeList::element_type * operand_10297 = _var_10297.firstObject () ;
    while ((operand_10297 != NULL)) {
      macroValidPointer (operand_10297) ;
      if (operand_10297->mType._isBuilt ()) {
        operand_10297->mType (HERE)->method_generateImplementation (_inLexique, var_cas_generatedCode3 COMMA_SOURCE_FILE_AT_LINE (230)) ;
      }
      operand_10297 = operand_10297->nextObject () ;
    }
    var_cas_generatedCode3._dotAssign_operation (GGS_string::constructor_CppLineComment (_inLexique COMMA_HERE)) ;
...
void routine_compileSemanticsComponentLL1 (C_Compiler & _inLexique,
  ...
  if (((var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (286)))._operator_strictSup (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    const GGS_stringlist _var_12352 = var_cas_typeMap.reader_unsolvedIndexKeyStringList (_inLexique COMMA_SOURCE_FILE_AT_LINE (293)) ;
    const GGS_stringlist::element_type * operand_12352 = _var_12352.firstObject () ;
    while ((operand_12352 != NULL)) {
      macroValidPointer (operand_12352) ;
      var_cas_m.writeString ("\n  - @") ;
      var_cas_m._dotAssign_operation (operand_12352->mValue) ;
  ...
  const GGS_typeMap _var_12550 = var_cas_typeMap ;
  const GGS_typeMap::element_type * operand_12550 = _var_12550.firstObject () ;
  while ((operand_12550 != NULL)) {
    macroValidPointer (operand_12550) ;
    if (operand_12550->mInfo.mType._isBuilt ()) {
      operand_12550->mInfo.mType (HERE)->method_checkNoCircularity (_inLexique, var_cas_typeMap.reader_count (_inLexique COMMA_SOURCE_FILE_AT_LINE (300)) COMMA_SOURCE_FILE_AT_LINE (300)) ;
    }
    operand_12550 = operand_12550->nextObject () ;
  }
  #ifdef DEBUG_TRACE_ENABLED
