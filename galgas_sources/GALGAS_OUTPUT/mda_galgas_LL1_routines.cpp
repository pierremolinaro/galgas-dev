...
void routine_compileSemanticsComponentLL1 (C_Compiler & _inLexique,
  ...
  GGS_typeMap  var_cas_typeMap ;
  ::routine_constructBuiltinTypeMap (_inLexique,  var_cas_typeMap COMMA_SOURCE_FILE_AT_LINE (224)) ;
  GGS_stringset  var_cas_importedComponentSet ;
  var_cas_importedComponentSet = GGS_stringset::constructor_emptySet (_inLexique COMMA_HERE) ;
  GGS_routineMap  var_cas_routineMap ;
  var_cas_routineMap = GGS_routineMap::constructor_emptyMap (_inLexique COMMA_HERE) ;
  GGS_typeList  var_cas_typeList ;
  _walk_throught_buildMapsForSemantics (_inLexique, var_cas_semanticsComponentRoot, var_cas_inSourceFile.ggs_string ().reader_stringByDeletingLastPathComponent (_inLexique COMMA_SOURCE_FILE_AT_LINE (229)), var_cas_endOfSourceFile, var_cas_importedComponentSet, var_cas_typeMap, var_cas_routineMap, var_cas_typeList) ;
  if (((var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (237)))._operator_strictSup (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    GGS_string var_cas_m ;
    var_cas_m = (var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (238)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (238)))._operator_concat (GGS_string (true, " type")) ;
    if (((var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (239)))._operator_isEqual (GGS_uint (true, 1U))).isBuiltAndTrue ()) {
      var_cas_m = GGS_string (true, "1 type is not defined:") ;
    }else{
      var_cas_m = (var_cas_typeMap.reader_unsolvedIndexCount (_inLexique COMMA_SOURCE_FILE_AT_LINE (242)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (242)))._operator_concat (GGS_string (true, " types are not defined:")) ;
    }
    const GGS_stringlist _var_10407 = var_cas_typeMap.reader_unsolvedIndexKeyStringList (_inLexique COMMA_SOURCE_FILE_AT_LINE (244)) ;
    const GGS_stringlist::element_type * operand_10407 = _var_10407.firstObject () ;
    while ((operand_10407 != NULL)) {
      macroValidPointer (operand_10407) ;
      var_cas_m.writeString ("\n  - @") ;
      var_cas_m._dotAssign_operation (operand_10407->mValue) ;
      operand_10407 = operand_10407->nextObject () ;
    }
    var_cas_endOfSourceFile.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (248)) ;
  }
  const GGS_typeMap _var_10605 = var_cas_typeMap ;
  const GGS_typeMap::element_type * operand_10605 = _var_10605.firstObject () ;
  while ((operand_10605 != NULL)) {
    macroValidPointer (operand_10605) ;
    if (operand_10605->mInfo.mType._isBuilt ()) {
      operand_10605->mInfo.mType (HERE)->method_checkNoCircularity (_inLexique, var_cas_typeMap.reader_count (_inLexique COMMA_SOURCE_FILE_AT_LINE (251)) COMMA_SOURCE_FILE_AT_LINE (251)) ;
    }
    operand_10605 = operand_10605->nextObject () ;
  }
  #ifdef DEBUG_TRACE_ENABLED
