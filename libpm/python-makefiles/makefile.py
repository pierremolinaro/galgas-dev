  ...
  def runJobs (self, maxConcurrentJobs, showCommand):
    if self.mErrorCount == 0:
      ...
      #--- Run
        ...
        loop = True
        returnCode = 0
        totalJobCount = len (self.mJobList)
        launchedJobCount = 0
        while loop:
            ...
            if (returnCode == 0) and (jobCount < maxConcurrentJobs):
              if (job.mState == 0) and (len (job.mRequiredFiles) == 0):
                ...
                if self.mShowProgressString:
                  progressString = "[{0:3d}/{1:3d}, {2:3d}%] ".format (launchedJobCount, totalJobCount, int (100.0 * float (launchedJobCount) / float (totalJobCount)))
                else:
                  progressString = ""
                #--- Run job
