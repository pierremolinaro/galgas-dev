...
//--- Get Count
  public: int32_t count (void) const { return mCount ; }
...
//--- Get capacity
  public: int32_t capacity (void) const { return mCapacity ; }
...
//--- Append objects at the end of the array
  ...
  public: void appendObjects (const int32_t inCount, const uint64_t inValue)  {
    if (inCount > 0) {
...
//--- Element access (with index checking)
  ...
  public: uint64_t operator () (const int32_t inIndex COMMA_LOCATION_ARGS) const  {
    #ifndef DO_NOT_GENERATE_CHECKINGS
  ...
  public: uint64_t & operator () (const int32_t inIndex COMMA_LOCATION_ARGS)  {
    #ifndef DO_NOT_GENERATE_CHECKINGS
      checkIndex (inIndex COMMA_THERE) ;
    #endif
    return mArray [inIndex] ;
