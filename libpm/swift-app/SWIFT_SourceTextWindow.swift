...
class SWIFT_SourceTextWindow : NSWindow, NSWindowDelegate { //, AutoLayoutTableViewDelegate {
...
//  private let mBaseView = AutoLayoutVerticalStackView ()
...
//  private var mBuildTextFontObserver : EBSimpleObserver? = nil
  ...
  init (withDocument inDocument : SWIFT_SourceTextDocument) {
//    self.mBuildOutputAttributeDictionary = [.font : gBuildWindowFont.propval]
    ...
    inDocument.mSourceTextManager.addView (self.mSourceTextView)
//    self.mMainHorizontalSplitView.autosaveName = "HSplitFor_" + (inDocument.fileURL?.path ?? "")
    self.delegate = self
    self.isReleasedWhenClosed = false
  //--- Build user interface
    self.setRootView (self.mSourceTextView)
...
//    )
  //---
    self.appendTab (document: inDocument, select: false)
//  //--- Open other tabs ?
...
//    }
    self.makeKeyAndOrderFront (true)
  }
...
//  }
  ...
  func appendTab (document inDocument : SWIFT_SourceTextDocument, select inSelect : Bool) {
    self.mTabArray.append (SWIFT_DisplayDescriptor (withDocument: inDocument))
...
//  func insertTab (document inDocument : SWIFT_SourceTextDocument, at inIndex : Int) {
...
//  }
  ...
  func tabsReferencing (document inDocument : SWIFT_SourceTextDocument) -> Int {
    var result = 0
