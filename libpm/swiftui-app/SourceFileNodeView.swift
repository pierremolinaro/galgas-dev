...
struct SourceFileNodeView : View {
  ...
  init (node inNode : SourceFileNode,
        selection inSelectionBinding : Binding <SourceFileNodeID?>) {
    self.mNode = inNode
    ...
    if self.mNode.isDirectory {
      DisclosureGroup (isExpanded: self.$mNode.mIsExpanded) {
        ForEach (self.mNode.mChildren, id: \.self.id) { child in
          SourceFileNodeView (node: child, selection: self.$mSelectionBinding)
          .padding (.leading, 4)
        }
      } label: {
        self.rowView ()
...
//                  print ("file: moving \(droppedURL) to \(targetURL)")
  ...
  private func rename (to inNewName : String) {
    ...
    do{
      ...
      let renamedFileID = SourceFileNodeID (url: newURL)
  ...
  private func duplicate () {
    do{
      ...
      let duplicatedFileID = SourceFileNodeID (url: destinationURL)
  ...
  //--- Select the new file
    let newFileID = SourceFileNodeID (url: destinationURL)
  ...
  private func newFolder () {
    ...
    //--- Select the new folder
      let newFolderID = SourceFileNodeID (url: destinationURL)
