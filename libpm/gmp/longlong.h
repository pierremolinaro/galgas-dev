...
#define count_leading_zeros(count, x) \
  do {									\
	...
	__a = __xr < ((UWtype) 1 << 2*__BITS4)				\
	  ? (__xr < ((UWtype) 1 << __BITS4) ? 1 : ((UWtype) (__BITS4 + 1)))		\
	  : (__xr < ((UWtype) 1 << 3*__BITS4) ? ((UWtype) (2*__BITS4 + 1))		\
	  : ((UWtype) (3*__BITS4 + 1)));						\
      }									\
    else								\
      {									\
	for (__a = ((UWtype) W_TYPE_SIZE) - 8; __a > 0; __a -= 8)			\
	  if (((__xr >> __a) & 0xff) != 0)				\
	    break;							\
	++__a;								\
      }									\
									\
    (count) = ((unsigned int) (W_TYPE_SIZE + 1 - __a - __clz_tab[__xr >> __a]));		\
  } while (0)
