...
/*--- Added by PM */
#ifdef _WIN32
  #include "32-mp_bases.h"
#else
  #if LLONG_MAX == INT32_MAX
    #include "32-mp_bases.h"
  #elif LLONG_MAX == INT64_MAX
    #include "64-mp_bases.h"
  #else
    #error "LLONG_MAX is != 32 and != 64"
  #endif
#endif
/*#ifndef SIZEOF_MP_LIMB_T
  #error "Undefined SIZEOF_MP_LIMB_T"
...
/* --- #if changed to #ifdef by PM */
#ifdef WANT_TMP_NOTREENTRANT
struct tmp_marker
{
...
/* (n) changed to ((size_t) n) by PM */
#define TMP_ALLOC_TYPE(n,type)  ((type *) TMP_ALLOC (((size_t) n) * sizeof (type)))
#define TMP_SALLOC_TYPE(n,type) ((type *) TMP_SALLOC (((size_t) n) * sizeof (type)))
#define TMP_BALLOC_TYPE(n,type) ((type *) TMP_BALLOC (((size_t) n) * sizeof (type)))
...
/* TMP_ALLOC_LIMBS ((xsize) + (ysize)) changed tp TMP_ALLOC_LIMBS ((size_t) ((xsize) + (ysize))) by PM */
#define TMP_ALLOC_LIMBS_2(xp,xsize, yp,ysize)				\
  do {									\
	...
	(yp) = TMP_ALLOC_LIMBS (ysize);					\
      ...
      {									\
	(xp) = TMP_ALLOC_LIMBS ((size_t) ((xsize) + (ysize)));			\
	(yp) = (xp) + (xsize);						\
      }									\
  } while (0)
...
#define __GMP_REALLOCATE_FUNC_TYPE(p, old_size, new_size, type)		\
  ((type *) (*__gmp_reallocate_func)					\
   (p, ((size_t) old_size) * sizeof (type), ((size_t) new_size) * sizeof (type)))
...
#define DIGITS_IN_BASEGT2_FROM_BITS(res, nbits, b)			\
  do {									\
    mp_limb_t _ph ; mp_limb_t _dummy __attribute__((unused));						\
    size_t _nbits = (nbits);						\
...
/* int __lb_base, __cnt; changed to unsigned long	__lb_base, __cnt; by PM */
#define MPN_SIZEINBASE(result, ptr, size, base)				\
  do {									\
    unsigned long	   __lb_base, __cnt;						\
    size_t __totbits;							\
									\
    ASSERT ((size) >= 0);						\
    ASSERT ((base) >= 2);						\
...
/* int __cnt changed to unsigned long __cnt by PM */
...
#define invert_limb(invxl,xl)						\
  do {									\
    mp_limb_t _dummy __attribute__ ((unused)) ;							\
    ASSERT ((xl) != 0);							\
...
#define DIGITS_IN_BASE_PER_LIMB(res, nlimbs, b)				\
  do {									\
    mp_limb_t _ph ; mp_limb_t _pl __attribute__((unused));							\
    umul_ppmm (_ph, _pl,						\
...
mpn_toom6h_mul_itch (mp_size_t an, mp_size_t bn) {
  ...
  estimatedN = (an + bn) / (mp_size_t) 10 + 1; /* Modified by PM : size_t -> mp_size_t */
...
mpn_toom8h_mul_itch (mp_size_t an, mp_size_t bn) {
  mp_size_t estimatedN;
  estimatedN = (an + bn) / (mp_size_t) 14 + 1;  /* Modified by PM : size_t -> mp_size_t */
...
{
  mp_size_t n = 1 + (2 * an >= 3 * bn ? (an - 1) / (mp_size_t) 3 : (bn - 1) >> 1);  /* Modified by PM : size_t -> mp_size_t */
...
{
  mp_size_t n = 1 + (3 * an >= 4 * bn ? (an - 1) >> 2 : (bn - 1) / (mp_size_t) 3);  /* Modified by PM : size_t -> mp_size_t */
...
{
  mp_size_t n = 1 + (2 * an >= 5 * bn ? (an - 1) / (mp_size_t) 5 : (bn - 1) >> 1);  /* Modified by PM : size_t -> mp_size_t */
  return 6*n + 4;
}
...
{
  mp_size_t n = 1 + (3 * an >= 5 * bn ? (an - 1) / (mp_size_t) 5 : (bn - 1) / (mp_size_t) 3);  /* Modified by PM : size_t -> mp_size_t */
  return 10 * n + 10;
}
...
{
  mp_size_t n = 1 + (an >= 3 * bn ? (an - 1) / (mp_size_t) 6 : (bn - 1) >> 1);  /* Modified by PM : size_t -> mp_size_t */
  return 10 * n + 10;
}
...
{
  mp_size_t n = 1 + (an >= 2 * bn ? (an - 1) / (mp_size_t) 6 : (bn - 1) / (mp_size_t) 3);  /* Modified by PM : size_t -> mp_size_t */
  return 9 * n + 3;
}
...
{
  mp_size_t n = 1 + (4 * an >= 5 * bn ? (an - 1) / (mp_size_t) 5 : (bn - 1) / (mp_size_t) 4);  /* Modified by PM : size_t -> mp_size_t */
  return 9 * n + 3;
}
