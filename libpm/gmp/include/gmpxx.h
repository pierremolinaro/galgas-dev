...
inline void __mpz_set_si_safe(mpz_ptr p, long l)
{
  if(l < 0)
  {
    ...
    __mpz_set_ui_safe(p, (unsigned long) l);
...
struct __gmp_binary_plus
{
  ...
  {
    if (l >= 0)
      mpf_add_ui(f, g, (unsigned long) l);
    else
...
struct __gmp_binary_minus
{
  ...
  {
    if (l >= 0)
      mpf_sub_ui(f, g, (unsigned long) l);
    else
...
// gcc-3.3 doesn't have __builtin_ctzl. Don't bother optimizing for old gcc.
    ...
    if (__GMPXX_CONSTANT(l) && (l & (l-1)) == 0)
    {
      ...
      {
        __gmp_binary_lshift::eval(z, w, (unsigned long) __builtin_ctzl(l));
      }
    }
    else
#endif
  ...
  static void eval(mpq_ptr q, mpq_srcptr r, unsigned long int l)
  {
	...
	mpq_set_ui(q, 0, 1);
      ...
      {
        __gmp_binary_lshift::eval(q, r, (unsigned long) __builtin_ctzl(l));
      }
    }
    else
#endif
  ...
  {
    if (l >= 0)
      mpf_mul_ui(f, g, (unsigned long) l);
    else
      {
...
struct __gmp_binary_divides
{
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, unsigned long int l)
  {
    ...
    if (__GMPXX_CONSTANT(l) && (l & (l-1)) == 0 && l != 0)
    {
      if (l == 1)
      {
        ...
        mpz_tdiv_q_2exp(z, w, (unsigned long) __builtin_ctzl(l));
...
#if __GMP_GNUC_PREREQ(3, 4)
    if (__GMPXX_CONSTANT(l) && (l & (l-1)) == 0 && l != 0)
      __gmp_binary_rshift::eval(q, r, (unsigned long) __builtin_ctzl(l));
    else
#endif
    {
      __GMPXX_TMPQ_UI;
  ...
  {
    if (l >= 0)
      mpf_div_ui(f, g, (unsigned long) l);
    else
      {
  ...
  {
    if (l >= 0)
      mpf_ui_div(f, (unsigned long) l, g);
    else
      {
...
{
  char *str;
//  __gmp_alloc_cstring(char *s) { str = s; } // Deleted by PM
  __gmp_alloc_cstring(char *s) : str (s) {} // Added by PM
  ~__gmp_alloc_cstring()
  {
...
//--- Added by PM: no copy
  private : __gmp_alloc_cstring (const __gmp_alloc_cstring &) ;
  private : __gmp_alloc_cstring & operator = (const __gmp_alloc_cstring &) ;
};
...
public:
  ...
  mp_bitcnt_t get_prec() const { return mpf_get_default_prec(); }
//--- Added by PM
  public : __gmp_expr (const __gmp_expr<mpz_t, __gmp_urandomm_value> & inOperand) : 
  state (inOperand.state),
  range (inOperand.range) {
  }
  private : __gmp_expr<mpz_t, __gmp_urandomm_value> & operator = (const __gmp_expr<mpz_t, __gmp_urandomm_value> &) ;
};
