		   ...
		   mp_srcptr xp, mp_size_t n, mp_size_t hn, mp_ptr tp)
{
  ...
  cy = 0;
  DO_addlsh2 (xp2, xp + ((mp_size_t) (k-2)) * n, xp + ((mp_size_t) k) * n, hn, cy);
  if (hn != n)
    cy = mpn_add_1 (xp2 + hn, xp + ((mp_size_t) (k-2)) * n + hn, n - hn, cy);
  for (i = ((int) k) - 4; i >= 0; i -= 2)
  ...
  cy = 0;
  DO_addlsh2 (tp, xp + ((mp_size_t) (k-2)) * n, xp + ((mp_size_t) k) * n, n, cy);
  for (i = ((int) k) - 4; i >= 0; i -= 2)
