...
mpz_clrbit (mpz_ptr d, mp_bitcnt_t bit_idx)
{
  ...
  limb_idx = (mp_size_t) (bit_idx / GMP_NUMB_BITS);
  ...
  if (dsize >= 0) {
    if (limb_idx < dsize) {
      ...
      if (UNLIKELY ((dlimb == 0) + limb_idx == dsize)) { /* dsize == limb_idx + 1 */
	      ...
	      SIZ (d) = (int) limb_idx; // (int)  added by PM
	    }
	  }else{
    }
  }
      ...
      if (limb_idx < dsize)
	{
	  ...
	  if (limb_idx > zero_bound)
	      ...
	      if (dlimb == 0)
		{
		  ...
		  dsize += (mp_size_t) dp[dsize];

		  SIZ (d) = -(int) dsize; // (int)  added by PM
		}
	    }
/*	  else
	    ; */
      ...
      else
	{
	  ...
	  SIZ (d) = -(int) (limb_idx + 1); // (int) added by PM
