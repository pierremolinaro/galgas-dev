...
LineColumnContents String::lineAndColumnFromIndex (const int32_t inIndex) const {
  ...
  if (inIndex < receiverLength) {
    ...
    while ((idx < receiverLength) && parseLine) {
      while ((idx < receiverLength) && parseLine) {
        parseLine = UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) != '\n' ;
        idx += parseLine ;
      }
      if (idx < inIndex) {
        parseLine = true ;
...
int32_t String::indexFromLineAndColumn (const int32_t inLineNumber,
  ...
  while (line < inLineNumber) {
    while ((idx < length ()) && (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) != '\n')) {
      idx += 1 ;
    }
    line += 1 ;
  }
...
bool String::endsWithString (const String & inString) const {
  ...
  for (int32_t i=0 ; (i<inString.length ()) && result ; i++) {
    result = utf32AtIndex (i + offset COMMA_HERE) == inString.utf32AtIndex (i COMMA_HERE) ;
  }
  return result ;
}
  ...
  for (int32_t i=0 ; i<stringLength ; i++) {
    ...
    if (previousCharIsSubstituteChar) {
      if (UNICODE_VALUE (c) == UNICODE_VALUE (inCharacter)) {
        resultingString.appendUnicodeChar (inCharacter COMMA_HERE) ;
      }else{
        resultingString.appendString (inString) ;
        resultingString.appendUnicodeChar (c COMMA_HERE) ;
      }
...
int32_t String::lastOccurrenceIndexOfChar (const utf32 inChar) const {
  ...
  while ((result > 0) && notFound) {
    result -- ;
    notFound = UNICODE_VALUE (utf32AtIndex (result COMMA_HERE)) != UNICODE_VALUE (inChar) ;
  }
  if (notFound) {
    result = -1 ;
  }
...
String String::subString (const int32_t inStartIndex,
  ...
  if (inLength > 0) {
    ...
    for (int32_t i=inStartIndex ; i<last ; i++) {
      s.appendUnicodeChar (utf32AtIndex (i COMMA_HERE) COMMA_HERE) ;
    }
  }
  return s ;
}
...
String String::stringByCapitalizingFirstCharacter (void) const {
  ...
  if (receiver_length > 0) {
    s.appendUnicodeChar (unicodeToUpper (utf32AtIndex (0 COMMA_HERE)) COMMA_HERE) ;
...
String String::lowercaseString (void) const {
  ...
  for (int32_t i=0 ; i<receiver_length ; i++) {
    s.appendUnicodeChar (unicodeToLower (utf32AtIndex (i COMMA_HERE)) COMMA_HERE) ;
  }
  return s ;
}
...
//--- Trim left
  int32_t idx = 0 ;
  while ((idx < receiver_length) && ((UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) == ' ') || (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) == '\n'))) {
    idx += 1 ;
  }
//--- Trim and replace
  bool isCurrentlyTrimming = false ;
  while (idx < receiver_length) {
    const utf32 c = utf32AtIndex (idx COMMA_HERE) ;
...
String String::uppercaseString (void) const {
  ...
  s.setCapacity ((uint32_t) receiver_length) ;
  for (int32_t i=0 ; i<receiver_length ; i++) {
    s.appendUnicodeChar (unicodeToUpper (utf32AtIndex (i COMMA_HERE)) COMMA_HERE) ;
  }
  return s ;
}
...
bool String::isUnsignedInteger (void) const {
  ...
  for (int32_t i=0 ; (i < length ()) && ok ; i++) {
    const uint32_t c = UNICODE_VALUE (utf32AtIndex (i COMMA_HERE)) ;
    ok = (c >= '0') && (c <= '9') ;
  }
  return ok ;
}
...
uint32_t String::currentColumn (void) const {
  ...
  for (int32_t i=receiver_length-1 ; (i>=0) && ! found ; i--) {
    found = UNICODE_VALUE (utf32AtIndex (i COMMA_HERE)) == '\n' ;
    if (! found) {
      result ++ ;
    }
  }
...
String String::stringWithRepeatedCharacter (const utf32 inRepeatedCharacter,
  ...
  for (uint32_t i=0 ; i<inCount ; i++) {
    result.appendUnicodeChar (inRepeatedCharacter COMMA_HERE) ;
  }
  return result ;
}
...
void String::convertToSInt32 (int32_t & outResult,
  ...
  int32_t idx = 0 ;
  if (length () > 0) {
    const utf32 c = utf32AtIndex (0 COMMA_HERE) ;
    if (UNICODE_VALUE (c) == '-') {
      isPositive = false ;
      idx = 1 ;
...
//--- Mantissa
  while ((idx < length ()) && isdigit ((int) UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)))) {
    outDoubleValue *= 10.0 ;
    outDoubleValue += (double) (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) - '0') ;
    idx ++ ;
  }
//--- Fractional part
  double divisor = 1.0 ;
  if ((idx < length ()) && (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) == '.')) { // Dot
    idx ++ ;
    while ((idx < length ()) && isdigit ((int) UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)))) {
      divisor *= 10.0 ;
      outDoubleValue *= 10.0 ;
...
//--- Exponent ?
  if (idx < length ()) {
    switch (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE))) {
    ...
    //--- Exponent sign
      ...
      while ((idx < length ()) && isdigit ((int) UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)))) {
        exponentValue *= 10.0 ;
        exponentValue += (double) (UNICODE_VALUE (utf32AtIndex (idx COMMA_HERE)) - '0') ;
        idx ++ ;
      }
...
String String::identifierRepresentation (void) const {
  ...
  for (int32_t i=0 ; i<receiver_length ; i++) {
    ...
    if (isalpha ((int) UNICODE_VALUE (c))) {
      ...
      s.appendUnicodeChar (TO_UNICODE ('_') COMMA_HERE) ;
...
String String::fileNameRepresentation (void) const {
  ...
  for (int32_t i=0 ; i<receiverLength ; i++) {
    ...
    if (isdigit (nc) || islower (nc)) {
      ...
      s.appendUnicodeChar (TO_UNICODE ('+') COMMA_HERE) ;
      s.appendUnicodeChar (TO_UNICODE (uint32_t (tolower (nc))) COMMA_HERE) ;
    }else{
...
String String::utf8RepresentationEnclosedWithin (const utf32 inCharacter, const bool inEscapeQuestionMark) const {
  ...
  for (int32_t i=0 ; i<receiver_length ; i++) {
    ...
    if (UNICODE_VALUE (c) == '\\') {
      ...
      s.appendUnicodeChar ('\\' COMMA_HERE) ;
      s.appendUnicodeChar ('n' COMMA_HERE) ;
    }else if (UNICODE_VALUE (c) == '\r') {
      s.appendUnicodeChar ('\\' COMMA_HERE) ;
      s.appendUnicodeChar ('r' COMMA_HERE) ;
...
static String hex4 (const uint32_t inValue) {
  ...
  String result = "" ;
  result.appendUnicodeChar (TO_UNICODE (digit [(inValue >> 12) & 15]) COMMA_HERE) ;
  result.appendUnicodeChar (TO_UNICODE (digit [(inValue >>  8) & 15]) COMMA_HERE) ;
  result.appendUnicodeChar (TO_UNICODE (digit [(inValue >>  4) & 15]) COMMA_HERE) ;
  result.appendUnicodeChar (TO_UNICODE (digit [(inValue >>  0) & 15]) COMMA_HERE) ;
  return result ;
}
...
String String::utf8RepresentationWithUnicodeEscaping (void) const {
  ...
  for (int32_t i=0 ; i<receiver_length ; i++) {
    ...
    if (UNICODE_VALUE (c) == '\\') {
      ...
      s.appendUnicodeChar ('\\' COMMA_HERE) ;
      s.appendUnicodeChar ('u' COMMA_HERE) ;
  ...
  for (int32_t i=0 ; i<components.count () ; i++) {
    if ((i & 1) == 0) {
      ...
      for (int32_t j=0 ; j<components (i COMMA_HERE).length () ; j++) {
        codePoint *= 16 ;
        const uint32_t c = UNICODE_VALUE (components (i COMMA_HERE).utf32AtIndex (j COMMA_HERE)) ;
        if ((c >= '0') && (c <= '9')) {
          codePoint += c - '0' ;
        }else if ((c >= 'a') && (c <= 'f')) {
          codePoint += c + 10 - 'a' ;
      ...
      result.appendUnicodeChar (codePoint COMMA_HERE) ;
    }
  }
  return outOk ? result : "" ;
}
...
int32_t String::compare (const String & inString) const {
  ...
  for (int32_t i=0 ; (i < minLength) && (result == 0) ; i++) {
    const uint32_t left = UNICODE_VALUE (utf32AtIndex (i COMMA_HERE)) ;
    const uint32_t right = UNICODE_VALUE (inString.utf32AtIndex (i COMMA_HERE)) ;
    if (left < right) {
      result = -1 ;
    }else if (left > right) {
      result = 1 ;
...
//--- Suppress training '/'
  while ((receiver_length > 1) && (UNICODE_VALUE (utf32AtIndex (receiver_length - 1 COMMA_HERE)) == '/')) {
    receiver_length -- ;
  }
//--- Search last '.'
  bool found = false ;
  ...
  while ((lastOccurrenceIndex > 0) && ! found) {
    lastOccurrenceIndex -- ;
    found = UNICODE_VALUE (utf32AtIndex (lastOccurrenceIndex COMMA_HERE)) == '.' ;
  }
  String result ;
...
//--- Suppress training '/'
  while ((usefulLength > 1) && (UNICODE_VALUE (utf32AtIndex (usefulLength - 1 COMMA_HERE)) == '/')) {
    usefulLength -= 1 ;
  }
//--- Search last '/'
  bool found = false ;
...
uint32_t String::LevenshteinDistanceFromString (const String & inOperand) const {
  ...
  for (int32_t j=1 ; j<=operandLength ; j++) {
    for (int32_t i=1 ; i<=myLength ; i++) {
      if (UNICODE_VALUE (utf32AtIndex (i-1 COMMA_HERE)) == UNICODE_VALUE (inOperand.utf32AtIndex (j-1 COMMA_HERE))) {
