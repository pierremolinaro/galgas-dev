...
void Lexique::enterIndexedValue (const uint32_t inIndexingKind,
  ...
  if ((nullptr != mIndexingDictionary) && inIndexedValue.isValid ()) {
    ...
    const String indexedKey = inIndexedValue.mProperty_string.stringValue () ;
    printf ("%u %u %u\n", tokenStartLocation, tokenLine, tokenLength) ;
    mIndexingDictionary->addIndexedKey (inIndexingKind,
                                        indexedKey,
                                        inIndexedValue.mProperty_location.sourceText ().sourceFilePath (),
                                        tokenLine,
                                        tokenStartLocation,
                                        tokenLength) ;
  }
...
void Lexique::enableIndexing (void) {
  macroMyNew (mIndexingDictionary, IndexingDictionary) ;
}
