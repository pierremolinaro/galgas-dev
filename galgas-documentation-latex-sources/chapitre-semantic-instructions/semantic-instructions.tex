...
\begin{itemize}
  ...
  \item \ggs=@stringlist=, elle représente une liste de suggestions, qui seront présentées à l'utilisateur dans l'ordre de la liste.
\end{itemize}
...
La suite présente trois exemples :
\begin{itemize}
...
Le fonctionnement de \ggs=before= et de \ggs=after= est analogue à celui de \ggs=replace= : on se reportera à l'exemple \ggs=replace= \refSubsubsectionPage {troisiemeExempleFixIt} pour ceux-ci.
...
Supposons qu'un identificateur ou qu'une chaîne ne puisse prendre qu'un ensemble de valeurs possibles. C'est le rôle des écritures sémantiques de vérifier que la valeur est l'une de celles autorisées.
...
rule <règle> ... {
  ...
  $literat_string$ ?let valeurÀVérifier
  ...
}
\end{galgas}
...
\begin{galgas}
  ...
  if [valeursPossibles hasKey !valeurÀVérifier] then
    ...
    error valeurÀVérifier : "valeur interdite"
  end
}
\end{galgas}
...
\begin{galgas}
  ...
  if [valeursPossibles hasKey !valeurÀVérifier] then
    ...
    error valeurÀVérifier : "valeur interdite" fixit {
