...
set -x
DIR=`dirname $0` &&
BUILD_DIR=galgas-book-gray-build-dir &&
cd $DIR &&
rm -f galgas-book-gray.pdf &&
rm -fr $BUILD_DIR &&
mkdir $BUILD_DIR &&
#--- First pass
/usr/texbin/pdflatex --file-line-error -output-directory=$BUILD_DIR '\newcommand\sortieEnCouleur{false}\input{galgas-book.tex}' &&
...
do
  ...
  /usr/texbin/makeindex -s $DIR/galgas-book-latex-inclusions/style-indexes.ist $BUILD_DIR/galgas-book.idx &&
  /usr/texbin/pdflatex --file-line-error -output-directory=$BUILD_DIR '\newcommand\sortieEnCouleur{false}\input{galgas-book.tex}' &&
  iteration=$((iteration+=1))
done &&
cp $BUILD_DIR/galgas-book.pdf galgas-book-gray.pdf &&
rm -fr $BUILD_DIR &&
