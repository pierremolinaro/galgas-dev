...
The \emph{variant\_expression} is an \nomType{uint} expression that ensures the loop is not endless: the \emph{variant\_expression} is computed at the beginning of the loop execution, and is decremented by one at the end of every iteration. Il it reaches zero, a run-time error is raised.

The \emph{expression} is an \nomType{bool} expression that expresses repetitive execution.
