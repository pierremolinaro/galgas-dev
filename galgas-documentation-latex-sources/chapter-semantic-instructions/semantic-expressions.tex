...
Une constante entière est une séquence de chiffres décimaux, éventuellement séparés par le caractère de soulignement \ggs+_+, et terminé par un suffixe. Ce suffixe détermine le type de la constante :
\begin{itemize}
  \item pas de suffixe : \ggs+@uint+ ;
  \item suffixe \ggs+S+ : \ggs+@sint+ ;
  \item suffixe \ggs+L+ : \ggs+@uint64+ ;
  \item suffixe \ggs+LS+ : \ggs+@sint64+.
\end{itemize}
...
\begin{table}[t]
  ...
  \ggs+@char+ & Initialisation au caractère \texttt{NULL} \\
  ...
  \ggs+@double+ & Initialisation à \texttt{0.0} \\
...
\begin{table}[t]
  ...
  8 (plus forte) & \ggs+.+ & Accès à un champ d'une structure & \refSubsectionPage{accesChampStructure}\\
                 & \ggs+`+ & Construction d'un object de type \ggs+@type+ & \\
  \end{tabular}
