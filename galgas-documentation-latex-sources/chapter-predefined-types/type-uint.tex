...
\contructeurSansArgument{errorCount}
{@uint}
{1.4.9}
{@uint}
...
\contructeurDeuxArguments{valueWithMask}
{@uint}
{1.6.12}
{@uint}
{@uint inLowerIndex}
...
\contructeurSansArgument{warningCount}
{@uint}
{1.4.9}
{@uint}
...
For example:\\
\texttt{[0xFFFF isUnicodeValueAssigned]} returns \motCle{false}, as \texttt{\textbackslash uFFFF} is not assigned.\\
\texttt{[0x41 isUnicodeValueAssigned]} returns \motCle{true}, as \texttt{\textbackslash u0041} is assigned (it is \emph{LATIN CAPITAL LETTER A}).
...
The \nomType{@uint} type supports the five arithmetic operators:\newline
