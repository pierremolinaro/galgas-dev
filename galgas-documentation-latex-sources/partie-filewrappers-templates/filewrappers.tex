...
Où :
\begin{itemize}
  \item \ggs+nom+ est le nom, interne à GALGAS, donné au \emph{filewrapper} ; ce nom doit être unique à chaque \emph{filewrapper} ;
  \item \ggs+"chemin"+ est le chemin du répertoire qui va être exploré récursivement au moment de la compilation ; c'est soit un chemin absolu (il commence par un \ggs+/+), soit un chemin relatif, par rapport au répertoire qui contient le fichier source qui déclare le \emph{filewrapper}.
\end{itemize}

La déclaration est divisée en trois parties délimitées par des accolades \ggs+{ ... }+ :
\begin{itemize}
