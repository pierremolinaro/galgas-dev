...
\optionGGS{-{-}generate-shared-map-automaton-dot-f{}iles} Exporte les automates d'états finis associés à chaque table de symboles de type \ggs!sharedmap!. Les fichiers de sortie sont placés dans le répertoire \texttt{build/helpers}, et portent le nom du type table postfixé par l'extension \texttt{.dot}.
...
\optionGGS{-{-}trace} Émet un fichier de trace de du compilateur. Format non documenté, utile uniquement pour déboguer le compilateur lui-même.
...
\optionGGS{-{-}generate-one-cpp-header} Engendre un seule fichier d'en-tête C++ pour tout le projet. Ceci permet un débogage plus simple du compilateur GALGAS lui-même, mais ralentit ensuite l'étape de compilation C++.


\optionGGS{-{-}check-gmp} Exécute au démarrage une série de calculs afin de vérifier si la librairie GMP est correctement compilée.
