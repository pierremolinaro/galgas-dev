...
\optionGGS{-{-}property-access-requires-self} Quand cette option est activée, \ggs!self! est requis pour accéder aux propriétés de l'objet courant dans les \emph{méthodes}, \emph{getter} et \emph{setter}. Par exemple, étant donné la classe \ggs!@maClasse!~:
...
Cette écriture du \ggs!setter! n'est acceptée que si l'option \texttt{-{-}property-access-requires-self} est désactivée. Si elle est activée, il faut écrire~:
\begin{galgas}
setter @maClasse incrémenter {
  self.maPropriété ++
}
...
\optionGGS{-{-}warns-anonymous-for-instruction} Quand cette option est activée, un warning est émis pour chaque instruction \ggs!for! dont la variable énumérée est anonyme (construction \ggs!for () in ... !, voir \refSubsectionPage{enumerationImplicite}).
