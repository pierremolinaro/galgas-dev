...
Par exemple, en GALGAS, les constantes entières acceptent le caractère \tpp{\_}, comme dans \ggs+123_456+. Elles peuvent être préfixées par \tpp{0x}, et postfixées par \tpp{S}, \tpp{LS} pour indiquer leur type~: \ggs+0x123_456S+, ou encore \ggs+0x_123_456_LS+. Le paquetage \tpp{listings} ne peut pas être paramétré pour afficher correctement les constantes entières de GALGAS.
...
\begin{itemize}
  \item à chaque \ggs+style+ défini dans l'analyseur lexical correspond une commande \LaTeX parti-culière~: par exemple à \tpp{keywordsStyle} correspond \tpp{\textbackslash keywordsStyle}\footnote{Si un suffixe est précisé (\texttt{-{}-mode{}:suffixe=latex}), alors ce suffixe est ajouté à la commande \tpp{\textbackslash keywordsStyle}.} (\refSubsectionPage{fonctionnement-mode-latex})~;
