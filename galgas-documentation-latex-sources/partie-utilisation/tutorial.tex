...
\begin{itemize}
  ...
  \item le répertoire \tpp{galgas-sources} contient les fichiers sources que vous allez compléter tout au long de ce tutorial ; son contenu est indiqué dans le tableau suivant.
\end{itemize}
...
\begin{description}
  \item[ ] \tpp{galgas -v chezmoi/logo/+logo.galgasProject}
\end{description}

L'option \tpp{-v} (verbose) provoque l'affichage de messages : observez ceux qui indiquent la création des fichiers C++. Ceux-ci sont rangés dans le répertoire \tpp{chezmoi/logo/build/output} et \tpp{chezmoi/logo/build/libpm}.
...
Travail à faire
écrivez l'analyseur lexical du langage LOGO. Après compilations GALGAS et C++, les exécutables \tpp{logo} et \tpp{logo-debug} obtenus sont partiellement opérationnels (pas encore d’analyseur syntaxique) : avec l'option \tpp{-{}-mode=lexical-only}, vous pouvez faire afficher la liste des symboles terminaux produite par l'analyse lexicale du fichier source passé en argument.
...
\subsubsection{Instructions sur les objets de type \tpp{class}}
