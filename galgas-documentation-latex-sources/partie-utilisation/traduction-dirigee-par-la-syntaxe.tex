...
L'instruction déclare une variable \galgas{s} de type \ggs+@string+ et lui affecte la chaîne traduite \footnote{Il existe des variantes pour exprimer l'obtention de la chaîne traduite, voir la description de l'instruction \galgas{grammar} à la \refSectionPage{instruction-grammar}.}.
...
\galgas{exp} est une expression de type \ggs+@string+. Son comportement est simple : la valeur de l'expression chaîne de caractères est simplement transmise à la chaîne produite.
