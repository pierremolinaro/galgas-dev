...
\begin{itemize}
  ...
  \item l'opérateur \galgas{\!=} ;
  \item le \emph{getter} \galgas{description} ;
  \item le \emph{getter} \galgas{dynamicType} ;
  \item le \emph{getter} \galgas{object}.
\end{itemize}
...
Le \emph{getter} \galgas{description} retourne une description textuelle du receveur, la même que celle affichée par l'instruction \galgas{log} (\refSectionPage{instructionLog}).
...
Le \emph{getter} \galgas{dynamicType} retourne un objet de type \galgas{@type}, dont la valeur représente le type dynamique du receveur (voir aussi la définition du type \refTypePredefini{type}).
...
Le \emph{getter} \galgas{object} retourne un objet de type \galgas{@object}. Une variable de type \refTypePredefini{object} peut encapsuler tout type de valeur.
