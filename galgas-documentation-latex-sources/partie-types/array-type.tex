...
Cette déclaration crée un tableau à $2*3*4$ éléments. Ces éléments sont par défaut \emph{invalides}, c'est à dire que leur lecture par le reader \galgas{valueAtIndex} déclenche une \emph{run-time error}. Pour être valide, un élément doit avoir été initialisé par un appel au modifier \galgas{setValueAtIndex}.
...
L'accès à la valeur d'un élément s'effectue par le reader \galgas{valueAtIndex}. La modification de la valeur d'un élément est réalisée par le modifier \galgas{setValueAtIndex} ou le modifier \galgas{forceValueAtIndex}.

\subsection{Le reader \texttt{valueAtIndex}}

Ce reader comporte \emph{dimension} arguments de type \galgas{@uint}, qui précisent l'indice pour chaque axe. Les indices sont comptés à partir de zéro (comme en C).
...
Les indices sont comptés à partir de zéro (comme en C). Contrairement au modifier \galgas{setValueAtIndex}, aucune \emph{run-time error} n'est déclenchée si un indice dépasse sa borne correspondante : le tableau est d'abord agrandi, ce qui ajoute des éléments invalides, puis l'élément désigné par les indices est affecté.
...
Le reader \galgas{axisCount} renvoie la dimension d'un tableau, c'est à dire le nombre de ces axes, le reader \galgas{sizeForAxis} renvoie la taille allouée à un axe particulier. Les modifiers \galgas{setSizeForAxis} et \galgas{setSize} permettent de modifier la taille d'un tableau.



\subsection{Le reader \texttt{axisCount}}

Ce reader sans argument renvoie un \galgas{@uint} qui contient le nombre d'axes d'un tableau. Comme ce nombre est fixé statiquement par la déclaration de type, la valeur retournée est toujours la même, pour toutes les objets d'un même type tableau.
...
array @monTableau : @string [3] ;
\end{lstlisting}
Pour tous les objets de type \galgas{@monTableau}, l'appel au reader \galgas{axisCount} renvoie la valeur $3$.
...
Ce modifier permet de changer la taille d'un axe sans changer les tailles attribuées aux autres axes. Il présente deux arguments de type \galgas{@uint} :
\begin{itemize}
...
Augmenter la taille fait apparaître de nouveaux éléments, qui sont invalides par défaut. Il faudra alors explicitement les initialiser individuellement par un appel au modifier \galgas{setValueAtIndex}.
...
Ce modifier permet de changer les tailles de tous les axes. Il présente \galgas{@uint} arguments de type \galgas{@uint} qui contiennent les nouvelles tailles de chaque axe.
...
Augmenter une taille fait apparaître de nouveaux éléments, qui sont invalides par défaut. Il faudra alors explicitement les initialiser individuellement par un appel au modifier \galgas{setValueAtIndex}.
