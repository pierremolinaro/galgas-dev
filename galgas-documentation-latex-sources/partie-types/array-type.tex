...
\begin{itemize}
  \item le type \ggst+@TypeElement+ qui cite le type de l'élément de tableau ;
  \item la dimension du tableau, qui doit être un nombre entier strictement positif ;
  \item le type \ggst+@TypeTableau+ qui est le nom donné au type de tableau.
\end{itemize}
...
Le seul constructeur d'un type tableau est le constructeur \ggst+new+. Il a pour but de fixer les dimensions initiales du tableau (il pourra ensuite être redimensionné). Il comporte \emph{dimension} arguments de type \ggst+@uint+, qui fixent la taille initiale de chaque axe.
Par exemple :
\begin{galgas3}
  @monTableau t [new !2 !3 !4] ;
\end{galgas3}

Cette déclaration crée un tableau à $2*3*4$ éléments. Ces éléments sont par défaut \emph{invalides}, c'est à dire que leur lecture par le getter \ggst+valueAtIndex+ déclenche une \emph{run-time error}. Pour être valide, un élément doit avoir été initialisé par un appel au setter \ggst+setValueAtIndex+.
...
Augmenter la taille fait apparaître de nouveaux éléments, qui sont invalides par défaut. Il faudra alors explicitement les initialiser individuellement par un appel au setter \ggst+setValueAtIndex+.
