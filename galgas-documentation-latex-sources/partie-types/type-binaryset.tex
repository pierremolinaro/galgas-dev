...
This operation returns un \galgas{@binaryset} value; do not confuse with \ggs+==+ operator that Retourne un \ggs+@bool+ value.
...
Returns a \galgas{@bool} value that indicates whether the receiver's value is the empty set :  \galgas{true} if receiver's value is the empty set, and \galgas{false} otherwise.
...
Returns a \galgas{@bool} value that indicates whether the receiver's value is the full set : \galgas{true} if receiver's value is the full set, and \galgas{false} otherwise.
...
Returns a string representation of the receiver's value. The returned string is compatible with the \lienConstructeur{binaryset}{binarySetWithPredicateString}.
...
Returns in an \galgas{@uint64} object the number of different values encoded by receiver, considering it uses \emph{inBitCount} bits. No overflow test in performed.
...
Theses operators require both arguments to be \galgas{@binaryset} objects, and return a \ggs+@bool+ object. These operations are very fast and are performed in a constant time (integer equality comparison).

Do not confuse with \refGetterPage{binaryset}{equalTo} and \refGetterPage{binaryset}{notEqualTo} that return a \galgas{@binaryset} object.
