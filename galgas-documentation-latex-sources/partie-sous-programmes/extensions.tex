...
For example, the following code add a getter to the \refTypePredefini{uint64} that computes the square of its value:
...
\lstinline[language=galgas]!string! whose type is \galgas{@string}. The following getter returns the value of this attribute, appended with the \lstinline[language=galgas]?" !"? string:
\begin{lstlisting}[language=galgas]
