...
//--------------------------------------------------------------------------------------------------
  ...
  //-------------------- Compiler
    ...
    while loop {
      ...
      let (s, status) = runHiddenCommand ("/usr/local/bin/galgas", ["--error-property-getter-call", "--no-color", "--max-errors=1", inCheminFichierGALGAS])
      if status == 0 {
    ...
    //    print (CYAN + s + ENDC)
        ...
        while (lineIndex < lines.count) && !found {
          found = lines [lineIndex].hasPrefix ("semantic error #1: property getter call (due to '--error-property-getter-call' option)")
          lineIndex += 1
        }
        loop = found
        if !loop {
...
//          print ("  Ligne Fixit : '\(lineFixIt)'")
          ...
          var appelGetter = ligneConcernée
          appelGetter.removeLast (ligneConcernée.count - dernierCaractère - 1)
          appelGetter.removeFirst (premierCaractère)
          print ("  Appel getter '\(appelGetter)'")
          if appelGetter.hasPrefix ("[") {
            appelGetter.removeFirst (1)
          }else{
            print (BOLD_RED + "Erreur, l'appel ne commence pas par '['" + ENDC)
            loop = false
          }
          if appelGetter.last == "]" {
            appelGetter.removeLast (1)
          }else{
            print (BOLD_RED + "Erreur, l'appel ne termine pas par ']'" + ENDC)
            loop = false
          }
          let a = appelGetter.components (separatedBy: " ")
          if a.count > 1 {
            var aa = a
            aa.removeLast ()
            appelGetter = aa.joined (separator: " ")
            appelGetter.append (".")
            appelGetter += a.last!
          }else{
            print (BOLD_RED + "Erreur, pas d'espace dans l'appel" + ENDC)
            loop = false
          }
          print ("  Appel modifié '\(appelGetter)'")
          let ligneModifiée = préfixe + appelGetter + suffixe
